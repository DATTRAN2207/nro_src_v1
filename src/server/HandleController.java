...
import assembly.CloneCharacter;
...
import thiendiabang.ThienDiaBang;
import thiendiabang.ThienDiaBangTileMap;
    ...
    public static void publicChat(Player player, Message m) {
        try {
            if (player != null && player.c != null && m != null && m.reader().available() > 0) {
                ...
                if (player.id == 1 && gm.length == 5 && gm[0].equals("i")) {
                    ...
                    itemup.sys = (byte) Integer.parseInt(gm[4]);
...
//                    }
                } else if ("xiu".equals(chat) && player.id == 1) {
                    Server.manager.TaiXiu[0].canthiep = 1;
                    player.sendAddchatYellow("Tài Xỉu Xu : Đã can thiệp thắng thành Xỉu ");
                } else if ("tai".equals(chat) && player.id == 1) {
                    Server.manager.TaiXiu[0].canthiep = 2;
                    player.sendAddchatYellow("Tài Xỉu Xu : Đã can thiệp thắng thành Tài ");
                } else if ("bt".equals(chat) && player.id == 1) {
                    Manager.serverChat("Thông Báo Sever", "Máy Chủ Sẽ Bảo Trì Sau Vài Giây Nữa! Vui Lòng Thoát Game Để Tránh Mất Dữ Liệu");
//                } else if ("b".equals(chat) && player.id == 1) {
//                    MenuAdmin.ShowAdmin(player);
                } else if ("a".equals(chat) && player.id == 1 || "a".equals(chat) && player.id == 126) {
                    player.typemenu = 125;
                    Menu.doMenuArray(player, new String[]{"Gửi Lượng", "Gửi Xu"});
                } else 
                if (player.id == 1 && gm.length == 2 && gm[0].equals("map")) {
                    ...
                    for (TileMap area : ma.area) {
                        if (area.numplayers < maxPlayers) {
                            ...
                            area.EnterMap0(player.c);
                            return;
                        }
                    }
               // } else if ("yen".equals(chat)) {
                    player.c.upyenMessage(500000000);
                //} else if ("xu".equals(chat)) {
                    player.c.upxuMessage(500000000);
              //  } else if ("luong".equals(chat)) {
                    player.upluongMessage(500000000);
              //  } else if (gm[0].equals("ngoc") && gm.length == 4) {
                    HandleController.addGem(player, gm);
                } else if (chat.equals("xoaht")) {
                    Service.startYesNoDlg(player, (byte) 20, "Bạn sẽ xoá sạch hành trang của chính mình?");
                } else {
                    m = new Message(-23);
    ...
    public static void worldChat(Player p, Message m) {
        try {
            if (p != null && p.conn != null && p.c != null && m != null && m.reader().available() > 0) {
                String chat = m.reader().readUTF();
                m.cleanup();
                if (p.c.level < 50 || p.c.xu < 10000000) {
                    p.conn.sendMessageLog("Yêu cầu trình độ cấp 50 và rương phải có 10 triệu xu mới có thể chát.");
                    return;
                }
                ...
                p.chatKTGdelay = System.currentTimeMillis() + 10000L;
                p.luongMessage(- 5);
    ...
    public static void wakeUpDieReturn(Player p) {
        if (p != null && p.conn != null && p.c != null && p.c.isDie && p.c.tileMap != null) {
            ...
            if (!tileMap.map.LangCo() && tileMap.map.dun == null) {
                if (tileMap.map.VungDatBiAn() || tileMap.map.MapVIP() || tileMap.map.MapSVIP() || tileMap.map.MapVVIP()) {
                    if (p.luong < 100) {
                        p.conn.sendMessageLog("Bạn không có đủ 100 lượng để hồi sinh!");
                        return;
                    }
                }
                if (p.luong < 1) {
    ...
    public static void move(Player p, Message m) {
        try {
            if (p != null && p.c != null && !p.c.get().isDie && m != null && m.reader().available() > 0) {
        ...
        try {
            if (p != null && p.c != null && p.conn != null && m != null && m.reader().available() > 0) {
                ...
                if (item != null && (!ItemTemplate.ItemTemplateId(item.id).isUpToUp || num > 0 && num <= item.quantity)) {
                    ...
                    if (data.type == 12 || data.type == 22 || ItemTemplate.isTypeNgocKham(item.id) || item.id == 383 || item.id == 384 || item.id == 385) {
                        p.conn.sendMessageLog("Vật phẩm quý giá bạn không thể bán được");
                        return;
                    }
                    if (item.id == 194 && p.c.level < 10) {
                        p.conn.sendMessageLog("Không Thể Bán Vật Phẩm Này");
                        return;
                    }
                    if (item.id == ItemName.GAY_MAT_TRANG && !item.isExpires || item.id == ItemName.GAY_TRAI_TIM && !item.isExpires
                            || item.id == ItemName.PET_BONG_MA && !item.isExpires || item.id == ItemName.PET_YEU_TINH && !item.isExpires
                            || item.id == ItemName.MAT_NA_HO && !item.isExpires || item.id == ItemName.MAT_NA_HO_1 && !item.isExpires || item.id == 843 || item.id == 844) {
                        p.conn.sendMessageLog("Vật Phẩm Có Giá Trị Không Thể Bán");
                        return;
                    }
                    item.quantity -= num;
                    if (item.quantity <= 0) {
            ...
            if (p != null && p.c != null && p.conn != null && m != null && m.reader().available() > 0) {
                ...
                switch (p.menuCaiTrang) {
                    case 0: {
                        ...
                        if (item != null) {
                            ItemTemplate data = ItemTemplate.ItemTemplateId(item.id);
                            int indexBag = p.c.getIndexBagid(item.id, item.isLock());
                            if (!item.isExpires && data.isUpToUp && indexBag != -1) {
                                ...
                                if (p.c.getBagNull() <= 0) {
                                    p.conn.sendMessageLog("Rương đồ không đủ chỗ trống");
                                    return;
                                }
                    ...
                    case 3:{
                        ...
                        if(item != null){
                            ...
                            p.c.indexTN = index;
                            Service.startYesNoDlg(p,(byte)96,String.format("Thăng Ngưỡng Khai Hoá Lên Cấp " + (cap + 1) + "\n - Cần %d Xu Và %d Lượng \n - Tỉ Lệ Thành Công: %s", xu[cap], luong[cap], tile[cap] + "%"));
                        }
                        break;
                    }
                    case 4: {
                        ...
                        if (item != null) {
                            ...
                            if (capkhaihoa >= capthangnguong) {
                                p.conn.sendMessageLog("Vui Lòng Thăng Ngương Khai Hoá Để Nâng Cấp ");
                                return;
                            }
                            if (capthangnguong < 1) {
                                p.conn.sendMessageLog("Vui Lòng Thăng Ngưỡng Trước Khi Nâng Cấp ");
                                return;
                            }
                            if (capkhaihoa >= 9) {
                                p.conn.sendMessageLog("Cấp Khai Hoá Đã Đạt Cấp Tối Đa");
                                return;
                            }
                            boolean flag = false;
                            for (Option option : item.options) {
                                if (option.id == 135 && option.param == 0) {
                                    flag = true;
                                    break;
                                }
                            }
                            if (flag) {
                                p.typemenu = 100;
                                Menu.doMenuArray(p,new String[]{"Sức Mạnh", "Phòng Ngự","Sinh Mệnh"});
                            }else {
                                for (Option option : item.options) {
                                    if (option.id == 148) {
                                    ...
                                    if (option.id == 149) {
                                        ...
                                        int[] tile = new int[]{70, 50, 40, 30, 25, 20, 10, 5, 2};
                                        int[] da = new int[]{5, 10, 20, 30, 40, 50, 60, 70, 80};
                                        Service.startYesNoDlg(p,(byte)106, String.format("Thăng Cấp Khai Hoá Lên Cấp " + (capkhaihoa + 1) + "\n - Cần %d Xu Và %d Lượng Và %d đá Phòng Ngự \n - Tỉ Lệ Thành Công: %s", xu[capkhaihoa], luong[capkhaihoa], da[capkhaihoa], tile[capkhaihoa] + "%"));
                                        break;
                                    }
                                    if (option.id == 163) {
                                        int[] xu = new int[]{1000000, 1500000, 2000000, 3000000, 4000000, 5000000, 7000000, 10000000, 20000000};
                                        ...
                                        int[] tile = new int[]{70, 50, 40, 30, 25, 20, 10, 5, 2};
                                        int[] da = new int[]{5, 10, 20, 30, 40, 50, 60, 70, 80};
                                        Service.startYesNoDlg(p,(byte)107, String.format("Thăng Cấp Khai Hoá Lên Cấp " + (capkhaihoa + 1) + "\n - Cần %d Xu Và %d Lượng Và %d đá Sát Thương \n - Tỉ Lệ Thành Công: %s", xu[capkhaihoa], luong[capkhaihoa], da[capkhaihoa], tile[capkhaihoa] + "%"));
                                        break;
                                    }
                                }
                            }
                    ...
                    case 5: {
                        Item im = p.c.ItemGiaHan.get(index);
                        ItemTemplate data = ItemTemplate.ItemTemplateId(im.id);
                        if (data.isTypeBijuu(im.id) || data.type == 27) {
                            p.conn.sendMessageLog("Vật phẩm không thể gia hạn");
                            return;
                        }
                        if (data.type == 33) {
                            if (p.c.quantityItemyTotal(ItemName.BUA_NORU) >= 1) {
                                if (im != null) {
                                    if (im.isExpires == true) {
                                        int sl = 0;
                                        boolean param164 = false;
                                        for (Option option : im.options) {
                                            if (option.id == 164) {
                                                sl = option.param;
                                                break;
                                            }
                                        ...
                                        if (sl >= 3) {
                                            p.conn.sendMessageLog("Sổ lần gia hạn đã đạt tối đa !");
                                            return;
                                        }
                                        for (Option option : im.options) {
                                            if (option.id == 164) {
                                        ...
                                        if (!param164) {
                                            im.options.add(new Option(1164, 1));
                                        }
                                        im.expires += 7776000000L;
                                        p.c.removeItemBags(871, 1);
                                        p.conn.sendMessageLog("Gia hạn thành công " + ItemTemplate.ItemTemplateId(im.id).name + " thêm 3 Tháng");
                                    } else {
                                        p.conn.sendMessageLog("Vật phẩm không có hạn sử dụng");
                                    }
                                } else {
                                    p.conn.sendMessageLog("Thao tác lỗi vui lòng thoát game và đăng nhập lại.");
                                }
                            } else {
                                p.conn.sendMessageLog("Không đủ Bùa Noru trong hành trang");
                            }
                        } else if (data.type == 2 || data.type == 11 || data.type == 12) {
                            if (p.c.quantityItemyTotal(ItemName.BUA_SOCHI) >= 1) {
                                if (im != null) {
                                    if (im.isExpires == true) {
                                        int sl = 0;
                                        boolean param164 = false;
                                        ...
                                        if (!param164) {
                                            im.options.add(new Option(164, 1));
                                        }
                                        im.expires += 7776000000L;
                                        p.c.removeItemBags(870, 1);
                                        p.conn.sendMessageLog("Gia hạn thành công " + ItemTemplate.ItemTemplateId(im.id).name + " thêm 3 tháng");
                                    } else {
                                        p.conn.sendMessageLog("Vật phẩm không có hạn sử dụng");
                                    }
                                ...
                                p.conn.sendMessageLog("Không đủ Bùa Sochi trong hành trang");
                            }
                        } else if (data.type != 2 && data.type != 11 && data.type != 12 && data.type != 33) {
                            if (p.c.quantityItemyTotal(ItemName.BUA_AITEMU) >= 1) {
                                if (im != null) {
                                    if (im.isExpires == true) {
                                        int sl = 0;
                                        boolean param164 = false;
                                ...
                                p.conn.sendMessageLog("Không đủ Bùa Aitemu trong hành trang");
                            }
                        }
                        if (!p.c.isTrade) {
                            Service.CharViewInfo(p, false);
                    ...
                    case 6: {
                        if (p.c.FruitUse == -1) {
                            ...
                            Service.CharViewInfo(p, false);
                            p.endLoad(true);
                            p.conn.sendMessageLog("sử dùng trái ác quỷ thành công");
                            break;
                        } else {
                            p.conn.sendMessageLog("Vui lòng xoa trái ác quỷ đang dùng trước");
                        }
                        break;
                    }
                    case 7: {
                        Item im = GmShop.ItemGmShop.get(index);
                        if (im != null) {
                            p.c.indexGmShop = index;
                            int coingm = 0;
                            String str = "Ngươi có muốn mua " + im.quantity +" " + ItemTemplate.ItemTemplateId(GmShop.ItemGmShop.get(index).id).name + " với giá ";
                            if (im.buyCoin > 0) {
                                coingm = im.buyCoin;
                                str += coingm + " coin không?";
                            }
                            Service.startYesNoDlg(p, (byte) 3_2, str);
                        }
                        break;
                    }
                }
...
//                        if ((item != null && (item.id == 35 || item.id == 37) || isalpha == false) && zoneid >= 0 && zoneid < tileMap.map.area.length && !tileMap.map.LangCo() && !tileMap.map.mapLDGT()) {
//                            tileMap.leave(p);
//                            tileMap.Enter(p);
//                            p.sendAddchatYellow("Không thể đổi khu bằng khả di lệnh ở đây.");
//                            return;
//                        }
                ...
                switch (type) {
                    ...
                    case 4: {
                        switch (p.menuCaiTrang) {
                            ...
                            case 3:
                                p.c.GetItemThangNguong();
                                if (index >= 0 && index < 32) {
                                    item = p.c.ItemTN[index];
                                }
                                break;
                            case 4:
                                p.c.GetItemThangNguong();
                            ...
                            case 6: {
                                if (index >= 0 && index < 30) {
                                    item = p.c.Fruit[index];
                                }
                                break;
                            }
                            case 7: {
                                if (index >= 0 && index < 126) {
                                    item = GmShop.ItemGmShop.get(index);
                                }
                                break;
                            }
                        }
                    ...
                    if (_char1 != null) {
                        ...
                        if (p == null) {
                            ...
                            if (player.c.tradeDelay > System.currentTimeMillis()) {
                                player.conn.sendMessageLog("Bạn cần " + (player.c.tradeDelay - System.currentTimeMillis()) / 1000L + "s để tiếp tục giao dịch.");
            ...
            if (player != null && player.c != null && player.conn != null && !player.c.isDie && m != null && m.reader().available() > 0) {
                if (player.c.tradeLock == 0) {
                    ...
                    for (i = 0; i < player.c.tradeIdItem.size(); ++i) {
                        Item item2 = player.c.getIndexBag(player.c.tradeIdItem.get(i));
                        if (item2 != null) {
    ...
    public static void submitTrade(Player player) {
        if (player != null && player.c != null && player.conn != null && !player.c.isDie) {
            if (player.c.tradeLock == 1) {
                ...
                boolean isErr = false;
                for (int index : n.tradeIdItem) {
                    Item item3 = n.p.c.getIndexBag(index);
                ...
                if (!isErr) {
                    for (int index : player.c.tradeIdItem) {
                        Item item3 = player.c.getIndexBag(index);
                        if (item3 == null || item3.isLock) {
                            isErr = true;
                            break;
                        }
                ...
                if (n.tradeCoin > n.xu || player.c.tradeCoin > player.c.xu || isErr) {
                    HandleController.closeLoad(player);
                    HandleController.closeLoad(n.p);
                    return;
                }
                Char c = player.c;
                ++c.tradeLock;
                ...
                try {
                    ...
                    if (n.tradeLock == 2) {
                        ...
                        for (i = 0; i < n.tradeIdItem.size(); ++i) {
                            item3 = n.p.c.getIndexBag(n.tradeIdItem.get(i));
                            if (item3 != null && !item3.isLock) {
                                a = a + ItemTemplate.ItemTemplateId(item3.id).name + " : Số Lượng : " + item3.quantity + "\n";
                                item1.add(item3);
                                n.removeItemBag(n.tradeIdItem.get(i));
                            }
                        }
                        for (i = 0; i < player.c.tradeIdItem.size(); ++i) {
                            item3 = player.c.getIndexBag(player.c.tradeIdItem.get(i));
                            if (item3 != null && !item3.isLock) {
                                item2.add(item3);
                                b = b + " " + ItemTemplate.ItemTemplateId(item3.id).name + " : Số Lượng : " + item3.quantity + "\n";
                                player.c.removeItemBag(player.c.tradeIdItem.get(i));
                            }
                        }
                        ...
                        LichSu.LichSuGiaoDich(player.c.name, n.name, b, player.c.tradeCoin, a, n.tradeCoin);
                        LichSu.LichSuGiaoDich(n.name, player.c.name, a, n.tradeCoin, b, player.c.tradeCoin);
    ...
    public static void selectMenuNpcTileMap(Player p, Message m) {
        try {
            if (p != null && p.c != null && p.conn != null && !p.c.isDie && m != null && m.reader().available() > 0) {
                Task.getTask(p.c, m.reader().readByte(), m.reader().readByte(), (byte) -1);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            ...
            try {
                if (player.c.isTrade) {
                    ...
                    if (n != null && n.p != null && n.isTrade) {
                        n.p.endDlg(true);
                        n.p.sendAddchatYellow("Giao Dịch Đã Bị Hủy Bỏ.");
                        player.sendAddchatYellow("Giao Dịch Đã Bị Hủy Bỏ.");
                    }
                    HandleController.closeTrade(player);
                }
                m = new Message(57);
            ...
            if (player != null && player.c != null && player.conn != null && !player.c.isDie && m != null && m.reader().available() > 0) {
                ...
                if (nF.equals(player.c.name)) {
                    ...
                    if (_char != null) {
                        ...
                        if (_char.taskId == 11 && _char.taskIndex == 1) {
                            _char.uptaskMaint();
                        }
                        if (friend.type == 1) {
    ...
    public static void viewInfoNinja(Player player, Message m) {
        try {
            if (player != null && player.c != null && player.conn != null && m != null && !player.c.isTrade && m.reader().available() > 0) {
                String playername = m.reader().readUTF();
                if(playername.contains("[S_VIP]")){
                    playername = playername.substring(7);
                }else if(playername.contains("[V_VIP]")){
                    playername = playername.substring(7);
                }else if(playername.contains("[VIP]")){
                ...
                if (playername.equals(player.c.name)) {
                    ...
                    if (n == null) {
                        ...
                        n.p.sendAddchatYellow(player.c.name + " đang xem thông tin của bạn");
...
// xem lôi đài
        ...
        try {
            if (player != null && player.c != null && player.conn != null && !player.c.isDie && m != null && m.reader().available() > 0 && player.menuIdAuction == -2) {
                ...
                if (item != null) {
                    ...
                    if (player.luong < 5) {
                        player.conn.sendMessageLog("Bạn không đủ lượng để bán vật phẩm");
                        return;
                    }
                    ItemTemplate data = ItemTemplate.ItemTemplateId(item.id);
                    if (data.type == 26) {
                    ...
                    LichSu.LichSuBanDoShinWa(player.c.name, data.name, xu, item.quantity);
                    player.c.ItemBag[index] = null;
                    player.c.upxuMessage(-5000);
                    LichSu.LichSuLuong(player.c.name, player.luong, (player.luong - 5), "bán đồ lên shinwa", -5);
    ...
    public static void viewItemAuction(Player player, Message m) {
        try {
            if (player != null && player.c != null && player.conn != null && !player.c.isDie && m != null && m.reader().available() > 0) {
                ...
                int itemID = m.reader().readInt();
                ShinwaTemplate item = ShinwaManager.findItem(player.menuIdAuction, itemID);
                if (item != null) {
                    Service.viewItemToAuction(player, itemID, item.getItem());
                } else {
                    System.err.println("Không tìm thấy vật phẩm");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static void buyItemAuction(Player player, Message m) {
        try {
            if (player != null && player.c != null && player.conn != null && !player.c.isDie && m != null && m.reader().available() > 0) {
                ...
                int itemID = m.reader().readInt();
                ShinwaTemplate itemShinwa = ShinwaManager.findItem(player.menuIdAuction, itemID);
                if (itemShinwa != null) {
                    ...
                    if (_char != null) { // Nhân vật nếu online trong game
                        _char.upxuMessage((long) itemShinwa.getPrice() * 99 / 100);
                        _char.p.conn.sendMessageLog("Bạn vừa nhận được " + itemShinwa.getPrice() * 99 / 100 + " Xu từ đồ đấu giá tại Shinwa.");
                    } else { // nếu nhân vật OFF
                    ...
                    ItemTemplate data = ItemTemplate.ItemTemplateId(itemShinwa.getItem().id);
                    LichSu.LichSuMuaDoShinWa(player.c.name, itemShinwa.getPrice(), data.name, itemShinwa.getItem().quantity);
                    ShinwaManager.entrys.get(player.menuIdAuction).remove(itemShinwa);
                    player.c.upxuMessage(-(long) itemShinwa.getPrice());
                    player.c.addItemBag(true, itemShinwa.getItem());
                    player.endDlg(true);
                } else {
                    player.conn.sendMessageLog("Vật phẩm không còn tồn tại!");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        try {
            if (player != null && player.c != null && player.conn != null && !player.c.isDie && m != null && m.reader().available() > 0) {
                ...
                switch (type) {
                    ...
                    return;
                    case 99: {
                        Mat.UpgradeMat(player, player.c.ItemBody[14], 0);
                        break;
                    }
                    case 1: {
                        Mat.UpgradeMat(player, player.c.ItemBody[14], 1);
                        break;
                    }
                    case -1: {
                        GameSrc.NangCapHanhTrang(player);
                        break;
                    }
                    case -2: {
                        GameSrc.BoQuaNhiemVu(player);
                        break;
                    }
                    case -3: {
                        GameSrc.DiDenLangShiba(player);
                        break;
                    }
                    case 3_2: {
                        Item im = GmShop.ItemGmShop.get(player.c.indexGmShop);
                        if (im != null) {
                            if (player.c.getBagNull() == 0) {
                            ...
                            if (im.buyCoin > 0) {
                                coin = im.buyCoin;
                                if (player.coin < coin) {
                                    player.conn.sendMessageLog("Ngươi không đủ coin để mua vật phẩm này!");
                                    return;
                                }
                                player.coin -= coin;
                                SQLManager.stat.executeUpdate("UPDATE `player` SET `coin`=" + player.coin + " WHERE `id`=" + player.id + " LIMIT 1;");
                                player.c.addItemBag(false, im);
                                break;
                            }
                        }
                        break;
                    ...
                    case -4: {
                        GameSrc.XoaBiKip(player);
                        break;
                    }
                    case -5: {
                        GameSrc.XoaAnToc(player);
                        break;
                    }
                    case 10: {
                        Boru.UpgradeBoru(player);
                        break;
                    }
                    case 11: {
                        UpgradePet.UpgradePet(player);
                        break;
                    }
                    case 12: {
                        Yoroi.UpgradeYoroi(player);
                        break;
                    }
                    case 15: {
                        BiKip.UpgradeBiKip(player);
                        break;
                    }
                    case 20: {
                        player.c.removeItemBag();
                        player.conn.sendMessageLog("Bạn đã xoá đồ hành trang thành công");
                        break;
                    }
                    case 29: {
                        AnToc.UpgradeAnToc(player);
                        break;
                    }
                    case 96: {
                        KhaiHoa.ThangNguong(player, player.c.indexTN);
                        break;
                    }
                    case 102:{
                        KhaiHoa.ThangNguong170(player,player.c.indexTN);
                        break;
                    }
                    case 103:{
                        KhaiHoa.ThangCap_ST(player,player.c.indexTN);
                        break;
                    }
                    case 104:{
                        KhaiHoa.ThangCap_PN(player,player.c.indexTN);
                        break;
                    }
                    case 105:{
                        KhaiHoa.ThangCap_SM(player,player.c.indexTN);
                        break;
                    }
                    case 106:{
                        KhaiHoa.ThangNguong171(player,player.c.indexTN);
                        break;
                    }
                    case 107:{
                        KhaiHoa.ThangNguong172(player,player.c.indexTN);
                        break;
                    }
                    //Mời ldgt
                    case 3: {
                            ...
                            if (clan != null && clan.ldgtID != -1) {
                                if (LanhDiaGiaToc.ldgts.containsKey(clan.ldgtID)) {
                                    ...
                                    if (lanhDiaGiaToc != null && lanhDiaGiaToc.map[0] != null && lanhDiaGiaToc.map[0].area[0] != null) {
                                        if (lanhDiaGiaToc.ninjas.size() <= 99) {
                    ...
                    case 5: {
                        if (player.c != null && player.c.clone != null && player.c.tileMap != null) {
                            ...
                            //vĩ thú
                            player.c.clone.mobBijuu = null;
                            player.c.clone.veff.removeAll(player.c.clone.veff);
                            player.c.clone.useTiemNang = 0;
                            player.c.clone.useKyNang = 0;
                            player.c.clone.useBanhPhongLoi = 0;
                            player.c.clone.useBanhBangHoa = 0;
            ...
            if (player != null && player.c != null && player.conn != null && m != null && m.reader().available() > 0) {
                if (player.c.get().nclass != 0) {
                    ...
                    if (quantity > 0 && quantity <= player.c.get().ppoint) {
                        ...
                        // Nhiệm vụ
                        if (player.c.taskId == 9 && player.c.taskIndex == 1) {
                            player.c.uptaskMaint();
                        }
                        player.c.get().ppoint -= quantity;
                        player.c.get().upHP(50);
                ...
                if (skill != null && player.c.get().spoint > 0 && point > 0) {
                    if (sk >= 67 && sk <= 72) {
                        ...
                        if (skill.point + point > data.maxPoint) {
                            ...
                            if (player.c.taskId == 9 && player.c.taskIndex == 2) {
                                if (point != 0) {
                                    player.c.uptaskMaint();
                                }
                            }
                    ...
                    if (player.c.ItemBag[i] != null && !(player.c.ItemBag[i]).isExpires && (ItemTemplate.ItemTemplateId(player.c.ItemBag[i].id)).isUpToUp) {
                        for (int j = (i + 1); j < player.c.ItemBag.length; j = (j + 1)) {
                            if (player.c.ItemBag[j] != null && !(player.c.ItemBag[i]).isExpires && (player.c.ItemBag[j]).id == (player.c.ItemBag[i]).id && (player.c.ItemBag[j]).isLock() == (player.c.ItemBag[i]).isLock()) {
                                if (player.c.ItemBag[i].quantity + player.c.ItemBag[j].quantity <= 30000) {
                                    (player.c.ItemBag[i]).quantity += (player.c.ItemBag[j]).quantity;
        ...
        try {
            if (player != null && player.conn != null) {
                ...
                if (m != null && player.c == null) {
                    String name = m.reader().readUTF();
                    String vip = "";
                    if(name.contains("[S_VIP]")){
                        name = name.substring(7);
                    }else if (name.contains("[V_VIP]")){
                        name = name.substring(7);
                    ...
                    for (i = 0; i < player.sortNinja.length; ++i) {
                        if (name.equals(vip + player.sortNinja[i])) {
                            player.c = Char.setup(player,vip + player.sortNinja[i]);
                            if (player.c != null) {
                                Client.gI().put(player.c);
                                player.sendInfo();
                                UpgradePet.checkbugbirehanh(player);
                                Manager.alert.sendAlert(player);
                                m = new Message(-23);
                                m.writer().writeInt(player.c.get().id);
                                m.writer().writeUTF("Chào mừng bạn đến với Máy Chủ NSO Kingseven . Website Chính Thức : nsokingseven.pro.");
                                if (player.c.isVIP == 3) {
                                    Service.chatKTG("S-VIP Vua Loạn Đấu " + player.c.name + " Đã Đăng Nhập Vào Game - Hãy Tránh Xa !");
                                } else if (player.c.isVIP == 2) {
                                    Service.chatKTG("Chào Mừng : V-VIP " + player.c.name + " Đã Đăng Nhập Vào Game!");

                                } else if (player.c.vip >= 3) {
                                    Service.chatKTG("Chào Mừng : VIP " + player.c.vip + " " + player.c.name + " Đã Đăng Nhập Vào Game!");
                                }
                                m.writer().flush();
                                player.conn.sendMessage(m);
                                m.cleanup();
                                if (player.c.taskIndex != -1) {
                                    Service.getTask(player.c);
                                    Task.requestLevel(player.c);
                                }
                                //vĩ thú
                                player.onBijuuInfo(player.c.get().id, player.c.get().bijuu);
                                break;
                            }
                        }
                    }
                    ...
                    try {
                        for (byte j = 0; j < player.sortNinja.length; ++j) {
                            if (player.sortNinja[j] != null) {
                                synchronized (Server.LOCK_MYSQL) {
                                    red = SQLManager.stat.executeQuery("SELECT `gender`,`name`,`class`,`level`,`head`,`ItemBody` ,`isVIP`FROM `ninja` WHERE `name`LIKE'" + player.sortNinja[j] + "';");
                                    if (red != null && red.first()) {
                                        String vip = "";
                                        if(red.getString("isVIP").equals(3)){
                                            vip = "[S_VIP]";
                                        }else if(red.getString("isVIP").equals(2)){
                                            vip = "[V_VIP]";
                                        }else if(red.getString("isVIP").equals(1)){
                                            vip = "[VIP]";
                                        }
                                        m.writer().writeByte(red.getByte("gender"));
                                        m.writer().writeUTF(vip + red.getString("name"));
                ...
                if (player.sortNinja[2] == null) {
                    ...
                    if (Util.CheckString(name, "^[a-zA-Z0-9]+$") && name.length() >= 5 && name.length() <= 15) {
                        if (player.sortNinja[0] != null) {
                            player.conn.sendMessageLog("Mỗi Tài Khoản Chỉ Được Tạo Tối Đa 1 Nhân Vật!");
                        } else {
                            synchronized (Server.LOCK_MYSQL) {
                                ...
                                red.close();
                                //vĩ thú
                                SQLManager.stat.executeUpdate("INSERT INTO ninja(`name`,`gender`,`head`,`ItemBag`,`ItemBox`,`ItemBST`,`ItemCaiTrang`,`Fruit`,`ItemBody`,`ItemMounts`,`bijuu`, `friend`, `effect`, `clan`, `exptype`, `skill`) VALUES (\"" + name + "\"," + gender + "," + head + ",'[]','[]','[]','[]','[]','[]','[]','[]','[]','[]','[]', 1,'[{\"id\": 0, \"point\": 0}]'       );");
                                byte i = 0;
                                while (true) {
                        ...
                        player.conn.sendMessageLog("Tên nhân vật chỉ chứa các ký tự từ a-z,0-9 và chiều dài từ 5 đến 15 ký tự!");
                    }
                }
            }
        } catch (Exception e) {
    ...
    public static void sendDataMap(Player player) {
        if (player.conn.version >= 211) {
                Server.manager.sendMap211(player);
            } else {
                Server.manager.sendMap(player);
            }
    }

    public static void sendDataSkill(Player player) throws IOException {
        if (player != null && player.conn != null) {
        ...
        try {
            if (player != null && player.conn != null && player.c != null && player.c.tileMap != null && m != null && m.reader().available() > 0) {
                if (player.c.tileMap.map.lanhDiaGiaToc != null && player.c.tileMap.map.mapLDGT()) {
                    if (player.c.tileMap.map.lanhDiaGiaToc.ninjas.size() >= 99) {
    ...
    public static void rewardedCave(Player player) {
        if (player != null && player.c != null && player.conn != null) {
            ...
            if (player.c.isHangDong6x == 1) {
                num = player.c.pointCave / 30;
            }
            if (player.c.IsLevel6x == 1 && player.c.PointMax < player.c.pointCave) {
                player.c.PointMax = player.c.pointCave;
            }
            // Bxh điểm hang
            if (num > 0) {
                if (player.c.getBagNull() == 0) {
                ...
                if (player.c.level < 50) {
                    ...
                    item = new Item();
                    item.id = 282;
                } else if (player.c.isHangDong6x == 2) {
                    item = new Item();
                    item.id = 891;
                } else {
                    item = new Item();
    ...
    public static void rewardedCT(Player player) { // set quà chiến trường
        if (player != null && player.c != null && player.conn != null) {
            switch (player.c.isTakePoint) {
                case 1: {
                    player.c.upxuMessage(500000);
                    player.upluongMessage(5000);
                    break;
                }
                case 2: {
                    player.c.upxuMessage(300000);
                    player.upluongMessage(3000);
                    break;
                }
                case 3: {
                    player.c.upxuMessage(200000);
                    player.upluongMessage(2000);
                    break;
                }
                case 4:
                case 5:
                ...
                case 9:
                case 10:
                    player.c.upxuMessage(100000);
                    player.upluongMessage(1000);
                    break;
                default: {
                    break;
                }
    ...
    public static void acceptClanDun(Player player, Message m) {
        try {
            if (player != null && player.conn != null && player.c != null && player.c.tileMap != null && m != null && m.reader().available() > 0) {
                if (player.c.tileMap.map.giaTocChien != null && player.c.tileMap.map.mapGTC()) {
                    if (!player.c.tileMap.map.giaTocChien.start) {
                        player.acceptClanDun(m);
                    } else {
                        player.conn.sendMessageLog("Đã mở cửa Gia Tộc Chiến, không thể mời thêm thành viên.");
                    }
                }
            }
        } catch (Exception e) {
    ...
    public static void thienDiaBang(Player player, Message m) {
        try {
            if (player != null && player.conn != null && player.c != null && m != null && m.reader().available() > 0) {
                ...
                switch (choose) {
                    case 0: {
                        player.conn.sendMessageLog("Đánh Đi Xem Thông Tin Làm Đéo Gì");
//                        String playername = m.reader().readUTF();
//                        m.cleanup();
//                        if (playername.equals(player.c.name)) {
//                            Service.sendInfoPlayers(player, player);
...
//                        }
                    ...
                    case 1: {
                        ...
                        if (player.c.delayJoinTDB > System.currentTimeMillis()) {
                            Service.chatNPC(player, (short) 4, "Con vừa thi đấu xong, 1 phút sau quay lại thi đấu tiếp nhé.");
                            return;
                        }
    ...
    public static void addGem(Player player, String[] args) {
        int i;
        int itemID = Integer.parseInt(args[1]);
        int upgrade = Integer.parseInt(args[2]);
        int soluong = Integer.parseInt(args[3]);
        boolean max = true;
        if (itemID == -1) {
            player.conn.sendMessageLog("Hãy nhập mã vật phẩm!");
            return;
        }

        if (soluong <= 0) {
            player.conn.sendMessageLog("Hãy nhập số lượng vật phẩm!");
            return;
        }

        if (player.c.getBagNull() < soluong) {
            player.conn.sendMessageLog("Hành không đủ chỗ trống!");
            return;
        }

        if (itemID != ItemName.HUYEN_TINH_NGOC && itemID != ItemName.HUYET_NGOC && itemID != ItemName.LAM_TINH_NGOC
                && itemID != ItemName.LUC_NGOC) {
            player.conn.sendMessageLog("Vật phẩm này không phải ngọc!");
            return;
        }

        if (upgrade < 1 || upgrade > 10) {
            player.conn.sendMessageLog("Cấp ngọc từ 1 đến 10!");
            return;
        }

        Item item = ItemTemplate.newGem(itemID, max);
        item.setLock(false);

        int[] paramUp = { 0, 250, 250, 250, 250, 250, 250, 250, 250, 250 };

        for (i = item.upgrade; i < upgrade; i++) {
            item.upgrade++;
            for (Option option : item.options) {
                switch (option.id) {
                    case 73:
                        // tấn công
                        option.param = (option.param > 0) ? option.param + paramUp[i] : -1;
                        break;
                    case 115:
                        // né đòn
                        option.param = (option.param > 0) ? option.param + 50 : -1;
                        break;
                    case 124:
                        // giảm trừ sát thương
                        option.param = (option.param > 0) ? option.param + 30 : -1;
                        break;
                    case 114:
                        // chí mạng
                        option.param = (option.param > 0) ? option.param + 5 : -1;
                        break;
                    case 126:
                        // phản đòn
                        option.param = (option.param > 0) ? option.param + 50 : -1;
                    ...
                    case 102:
                        // sát thương lên quái
                        option.param = (option.param > 0) ? option.param + 700 : -1;
                        break;
                    case 105:
                        // sát thương chí mạng
                        option.param = (option.param > 0) ? option.param + 500 : -1;
                        break;
                    case 103:
                    ...
                    case 229:
                        // hồi phục hp, mp
                        option.param = (option.param > 0) ? option.param + (2 * i) : -1;
                        break;
                    case 104:
                        int[] exp = { 0, 210, 510, 1010, 2010, 5010 ,10010, 20010, 50010, 100010 };
                        option.param = exp[i];
                        break;
                    case 123:
                        int[] giaKham = { 800000, 1200000, 1600000, 2000000, 2400000, 2800000, 3200000, 3600000,
                                4000000, 4400000 };
