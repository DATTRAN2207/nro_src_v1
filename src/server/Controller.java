...
//                    if (player.c.xu < 10000000) {
//                        player.conn.sendMessageLog("Yêu Cầu Trên 10 Triệu Xu Mới Có Thể Giao Dịch");
//                        return;
//                    }
                    if (player.c.level < 30) {
                        player.conn.sendMessageLog("Yêu Cầu Trình Độ Cấp 30 Mới Có Thể Giao Dịch");
                        return;
                    }
    ...
    private static void messageNotMap(Message message, Player player) {
        try {
            ...
            switch (b) {
                ...
                case 122: {
                    if (player != null && player.c != null && player.conn != null && !player.c.isDie && message != null && message.reader().available() > 0) {
                        byte type = message.reader().readByte();
                        byte id = message.reader().readByte();
                        if (type == 0) {
                            GameCanvas.sendImgAutoEffect(player, id);
                        }
                        if (type == 1) {
                            GameCanvas.sendDataAutoEffect(player, id);
                        }
                    }
                    break;
                }
