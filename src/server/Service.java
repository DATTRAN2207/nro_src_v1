...
public class Service {
    ...
    public static void addMob(Char _char, ArrayList<Mob> mobs) {
        Message msg = null;
        try {
            msg = new Message((byte) 122);
            msg.writer().writeByte(0);
            msg.writer().writeByte(mobs.size());
            for (short i = 0; i < mobs.size(); i = (short) (i + 1)) {
                Mob mob = mobs.get(i);
                if (mob != null) {
                    ...
                    if (_char.p.conn.version >= 211) {
                        msg.writer().writeShort(mob.templates.id);
                    } else {
    ...
    protected static void ClearCache(Session session) throws Exception {
        Message msg = messageNotLogin((byte) 2);
        session.sendMessage(msg);
        msg.cleanup();
    }
// TaskChínhTuyến

    public static void openUIMenu(Char _ninja, String[] menu) {
        Message msg = null;
        try {
    ...
    public static void getTask(final Char _char) {
        Message msg = null;
        try {
            final TaskTemplate taskTemplate = Manager.taskTemplates[_char.taskId];
            msg = new Message((byte) 47);
            msg.writer().writeShort(taskTemplate.taskId);
            msg.writer().writeByte(_char.taskIndex);
            ...
            msg.writer().writeShort(_char.taskCount);
    ...
    public static void updateTask(final Char _char) {
        Message msg = null;
        try {
    ...
    public static void nextTask(final Char _char) {
        Message msg = null;
        try {
    ...
    public static void finishTask(final Char _char) {
        Message msg = null;
        try {
    ...
    public static void openUIConfirm(Char c, short npcId, String chat, String[] menu) {
        Message msg = null;
        try {
    ...
    public static void openUISay(Char c, short npcId, String chat) {
        Message msg = null;
        try {
    ...
    public static void sendInfoChar(Player player, Player _p) {
        ...
        try {
            ...
            if (player.c.isVIP == 3) {
                m.writer().writeUTF("[S_VIP]" + player.c.name);
            } else if (player.c.isVIP == 2) {
                m.writer().writeUTF("[V_VIP]" + player.c.name);
            } else if (player.c.isVIP == 1) {
                m.writer().writeUTF("[VIP]" + player.c.name);
            } else {
    ...
    public static void sendclonechar(Player p, Player top) {
        try {
            ...
            //vĩ thú
            if (p.c.clone.mobBijuu != null) {
                m = new Message(-30);
                m.writer().writeByte(-68);
                m.writer().writeInt(p.c.clone.id);
                m.writer().writeByte(p.c.clone.mobBijuu.templates.id);
                m.writer().writeByte(p.c.clone.mobBijuu.isboss ? 1 : 0);
                m.writer().flush();
                top.conn.sendMessage(m);
                m.cleanup();
            }
    ...
    public static void CharViewInfo(Player p, boolean sendEff) {
        ...
        try {
            ...
            if (c.isVIP == 3) {
                msg.writer().writeUTF("[S_VIP]" + c.name);
            } else if (c.isVIP == 2) {
                msg.writer().writeUTF("[V_VIP]" + c.name);
            } else if (c.isVIP == 1) {
                msg.writer().writeUTF("[VIP]" + c.name);
            } else {
            ...
            int length = c.maxluggage;
            if (length > c.ItemBag.length) {
                length = c.ItemBag.length;
            }
            msg.writer().writeByte(length);
            byte j;
            for (j = 0; j < length; ++j) {
                if (c.ItemBag[j] == null) {
    ...
    public static void ENDWait(Char c) {
        try {
            Message m = messageSubCommand((byte) -89);
            m.writer().flush();
            c.p.conn.sendMessage(m);
            m.cleanup();
        } catch (Exception var3) {
    ...
    public static void sendLoiDaiList(Char c) {
        try {
            ...
            for (Dun dun : Dun.duns.values()) {
                if (dun != null) {
                    m.writer().writeByte(dun.dunID);
                    m.writer().writeUTF(dun.name1 + " (" + dun.lv1 + ")");
                    m.writer().writeUTF(dun.name2 + " (" + dun.lv2 + ")");
                } else {
                    System.out.println("dun is null");
                }
            }
            m.writer().flush();
            c.p.conn.sendMessage(m);
...
//    public static void SaveDuLieuNguoiChoi() {
//        int i;
//        Player player = null;
...
//    }
    ...
    public static void requestItemInfoMessage(Player player, Item item, int index, int typeUI) {
        ...
        try {
            ...
            if (item.id == 233 || item.id == 234 || item.id == 235) {
                ByteArrayOutputStream a = GameSrc.loadFile("res/assets/icon/1/diado.png");
                if (a != null) {
    ...
    public static void sendInfoPlayers(Player player, Player p) {
        ...
        try {
           ...
           if (p.c.isVIP == 3) {
                m.writer().writeUTF("[S_VIP]" + p.c.name);
            } else if (p.c.isVIP == 2) {
                m.writer().writeUTF("[V_VIP]" + p.c.name);
            } else if (p.c.isVIP == 1) {
                m.writer().writeUTF("[VIP]" + p.c.name);
            } else {
            ...
            m.writer().writeInt((int) p.c.get().dameMax());
            m.writer().writeInt((int) p.c.get().dameDown());
            m.writer().writeShort(p.c.get().Exactly());
            m.writer().writeShort(p.c.get().Miss());
            m.writer().writeShort(p.c.get().Fatal());
            m.writer().writeShort((int) p.c.get().ReactDame());
            ...
            m.writer().writeByte(p.c.countTaskHangNgay);
            m.writer().writeByte(2 - p.c.countTaskTaThu);
            m.writer().writeByte(p.c.nCave);
            m.writer().writeByte(p.c.get().useTiemNang);
            m.writer().writeByte(p.c.get().useKyNang);
            ...
            m.writer().writeByte(p.c.useBanhPhongLoi);
            m.writer().writeByte(p.c.useBanhBangHoa);
            m.writer().flush();
            player.conn.sendMessage(m);
            m.cleanup();
            if (!p.c.isTrade) {
    ...
    public static void openMenuThangNguong(Player p) {
        Message m = null;
        try {
            p.menuCaiTrang = 3;
            p.c.GetItemThangNguong();
            Service.sendTileAction(p, (byte) 4, "Thăng Ngưỡng", "Thăng Ngưỡng");
            m = new Message(31);
            m.writer().writeInt(p.c.xuBox);
            m.writer().writeByte(p.c.ItemTN.length);
            for (Item itemCT : p.c.ItemTN) {
                if (itemCT != null) {
                    m.writer().writeShort(itemCT.id);
                    m.writer().writeBoolean(itemCT.isLock);
                    if (ItemTemplate.isTypeBody(itemCT.id) || ItemTemplate.isTypeNgocKham(itemCT.id)) {
    ...
    public static void openMenuThangCap(Player p) {
        Message m = null;
        try {
            p.menuCaiTrang = 4;
            p.c.GetItemThangNguong();
            Service.sendTileAction(p, (byte) 4, "Thăng Cấp", "Thăng Cấp");
            m = new Message(31);
            m.writer().writeInt(p.c.xuBox);
            m.writer().writeByte(p.c.ItemTN.length);
            for (Item itemCT : p.c.ItemTN) {
    ...
    public static void openMenuGiaHan(Player p) {
        Message m = null;
        try {
            p.menuCaiTrang = 5;
            m = new Message(31);
            ...
            m.writer().writeInt(p.c.xuBox);
            m.writer().writeByte(p.c.ItemGiaHan.size());
            for (Item item : p.c.ItemGiaHan) {
                if (item != null) {
                    m.writer().writeShort(item.id);
                    m.writer().writeBoolean(item.isLock);
                    if (ItemTemplate.isTypeBody(item.id) || ItemTemplate.isTypeNgocKham(item.id)) {
