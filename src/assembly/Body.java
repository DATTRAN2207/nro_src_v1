...
public abstract class Body {
    ...
    public int getMaxHP() {
        ...
        hpmax += this.getPramItem(136);
        hpmax += this.getPramItem(159) * 10;//vĩ thú
        hpmax += (hpmax * this.getPramItem(138)/100);
    ...
    public int getMaxMP() {
        ...
        mpmax += this.getPramItem(137);
        mpmax += this.getPramItem(160) * 10;//vĩ thú
        if (this.mp > mpmax) {
            this.mp = mpmax;
        }
        return mpmax;
    ...
    public int eff5buffHP() { // 5s buff hp
        return this.getPramItem(30) + this.getPramItem(120);
    }

    public int eff5buffMP() { // 5s buff mp
    ...
    public long dameSide() {
        ...
        if (this.Side() == 1) {
            ...
            si += si * (this.getPramSkill(1) + this.getPramItem(9) + percent) / 120;
            si += this.getPramItem(1);
            si += this.getPramItem(0);
            si += this.getPramItem(160);//vĩ thú
        } else {
            si = this.getPotential(0);
            si += si * (this.getPramSkill(0) + this.getPramItem(8) + percent) / 120;
            si += this.getPramItem(0);
            si += this.getPramItem(1);
            si += this.getPramItem(157);//vĩ thú
        }
        si += this.getPramItem(38);//tan cong co ban vu khi
        eff = this.c.get().getEffId(17);
        if (eff != null) {
            si += this.getPramItem(38) * eff.param / 200;//buff tan cong co ban phai cung
        }
        eff = this.c.get().getEffId(19);
        if (eff != null) {
            si += this.getPramItem(38) * eff.param * 10 / 300;//buff tan cong co ban phai quat
        }
        si += this.getPramItem(38) * this.getPramSkill(11) / 200;// buff tan cong co ban cua skill chu dong
        eff = this.c.get().getEffId(25);
        if (eff != null) {
            si += this.getPramItem(38) * eff.param / 200;//buff tan cong co ban cua long luc dan
        }
        return si;
    }
    ...
    public float dameMin() {
        return this.dameMax() * 85 / 100;
    }
    ...
    public int ResFire() {
        ...
        bear += this.getPramItem(141);
        bear += (bear * this.getPramItem(127) / 120);
        return bear;
    }
    ...
    public int ResIce() {
        ...
        bear += this.getPramItem(81);
        bear += this.getPramItem(118);
        bear += this.getPramItem(142);
        bear += (bear * this.getPramItem(128) / 120);
        return bear;
    }
    ...
    public int ResWind() {
        ...
        bear += this.getPramItem(81);
        bear += this.getPramItem(118);
        bear += this.getPramItem(143);
        bear += (bear * this.getPramItem(129) / 120);
        return bear;
    }
    ...
    public int Exactly() {
        ...
        exa += this.getPramSkill(144);
        exa += this.getPramItem(158);//vĩ thú
        return exa;
    }
    ...
    public int Miss() {
        ...
        miss += this.getPramItem(158) * 150 / 350;//vĩ thú
    ...
    public int Fatal() {
        ...
        ft += this.getPramSkill(145);
        return ft;
    }

    public long percentFantalDameDown() {
        int pfdd = 0;
    ...
    public int getPotential(int i) {
        ...
        potential = potential * (100 + this.getPramItem(58)) / 200;
    ...
    public int getPramItem(int id) {
        try {
            ...
            //vĩ thú
            for (Item bijuu : this.c.get().bijuu) {
                if (bijuu != null) {
                    for (Option option : bijuu.options) {
                        if (option.id == id) {
                            param += option.param;
                        }
                    }
            ...
            if (this.FruitUse != -1 && this.isHuman) {
                for (Option option : this.c.Fruit[this.FruitUse].options) {
                    if (option.id == id) {
                        param += option.param;
                    }
                }
    ...
    public int getDame103() {
        if (this.c == null) {
            return 0;
        }
        int param = getPramItem(103);
        param += getPramItem(113);
        param += getPramItem(147);
        return param;
