...
public class Mob {
    ...
    private final ArrayList<Character> sortFight;
    private static final int[] arrMobLangCoId = new int[]{148, 146, 147, 148, 149, 151, 152, 154, 155, 156, 157, 159};
    private static final int[] arrMobChienTruongId = new int[]{90, 91, 92, 93, 94, 95, 96, 97, 98, 99};
    ...
    public void updateHP(int num, int _charId, boolean liveAttack) {
        ...
        if (this.hp <= 0) {
            ...
            // Time hồi sinh
            if (this.isRefresh) {
                this.timeRefresh = System.currentTimeMillis() + 15000L;
            }
            if (this.isRefresh && this.checkMobLangCo()) {
                ...
                this.timeRefresh = System.currentTimeMillis() + 100000L;
    ...
    public void handleAfterCharFight(Char _char) {
        ...
        if (this.tileMap.map.mapLDGT()) {
            ...
            if (this.templates.id == 98) {
                _char.pointCT += 500;
                Service.updatePointCT(_char, 500);
                this.isRefresh = false;
                Manager.serverChat("Chiến Trường", "Bạch Long Trụ đã bị phá hủy ");
            } else if (this.templates.id == 99) {
                _char.pointCT += 500;
                Service.updatePointCT(_char, 500);
                this.isRefresh = false;
                Manager.serverChat("Chiến Trường", "Hắc Long Trụ đã bị phá hủy");
            }
            ...
            if (Math.abs(this.level - _char.get().level) <= 10 || this.tileMap.map.LangCo() || this.tileMap.map.mapChienTruong()) {
                ...
                if (Server.manager.event != 0 && !this.tileMap.map.MapVIP()) {
                    ItemLeave.leaveItemSuKien(this.tileMap, this, master);
                }
                if (Server.manager.event != 0 && !this.tileMap.map.VungDatBiAn()) {
                    ItemLeave.leaveItemSuKien(this.tileMap, this, master);
                }
                if (Server.manager.event != 0 && !this.tileMap.map.MapSVIP()) {
                    ItemLeave.leaveItemSuKien(this.tileMap, this, master);
                }
                if (Server.manager.event != 0 && !this.tileMap.map.MapVVIP()) {
                    ItemLeave.leaveItemSuKien(this.tileMap, this, master);
                }
                switch (Util.nextInt(1, 2)) {
                    case 1: {
                        if (this.lvboss == 0 && Util.nextInt(70) < 5) { // tỉ lệ rơi yên
                            ItemLeave.leaveYen(this.tileMap, this, master);
                        }
                        break;
                    }
                    case 2: {
                        if (Util.nextInt(100) < 3 && this.tileMap.map.id != 161 && !this.tileMap.map.LangCo() && !this.tileMap.map.MapVIP() && !this.tileMap.map.MapSVIP() && !this.tileMap.map.MapVVIP()) {
                            ItemLeave.LeaveItemMapNgoai(this.tileMap, this, master);
                        }
                        break;
                    }
                }
            }
            if (Math.abs(this.level - _char.get().level) <= 20) {
                if (Server.manager.event != 0 && this.tileMap.map.VungDatBiAn()) {
                    ItemLeave.leaveItemSuKien(this.tileMap, this, master);
                }
            } 
            if (this.tileMap.map.VDMQ() && (_char.get().getEffId(40) != null || _char.get().getEffId(41) != null) && Math.abs(this.level - _char.get().level) <= 10) {
                ItemLeave.LeaveItemMapVDMQ(tileMap, this, master);
            } else if (this.tileMap.map.MapVIP()) {
               if (_char.ItemBody[24] != null && _char.ItemBody[24].id == 892) {
                    _char.p.upluongMessage(2);
                } else {
                    _char.p.upluongMessage(1);
                }
                ItemLeave.LeaveItemMapVIP(tileMap, this, master);
            } else if (this.tileMap.map.LangCo()) {
                ItemLeave.LeaveItemMapLangCo(tileMap, this, master);
            } else if (this.tileMap.map.VungDatBiAn()) {
                if (Util.nextInt(100) < 30) {
                    if (_char.ItemBody[24] != null && _char.ItemBody[24].id == 892) {
                        _char.p.upluongMessage(5);
                        _char.upxuMessage(Util.nextInt(1000, 10000));
                    } else {
                        _char.p.upluongMessage(3);
                    }
                } else {
                    ItemLeave.LeaveItemMapBiAn(tileMap, this, master);
                }
            } else if (this.tileMap.map.MapSVIP()) {
                _char.p.upluongMessage(1);
                ItemLeave.LeaveItemMapSVIP(tileMap, this, master);
            } else if (this.tileMap.map.MapVVIP()) {
                _char.p.upluongMessage(1);
                ItemLeave.LeaveItemMapVVIP(tileMap, this, master);
            }
            if (this.isboss) {
                if (this.tileMap.map.cave == null) {
                    for (int i = 0; i < 20; i++) {
                    ...
                    switch (this.templates.id) {
//                        case 144:
//                            //     ItemLeaveBoss.ItemBossSuKien(this.tileMap, this, master);
//                            break;
//                        case 210:
//                            //  ItemLeaveBoss.ItemBossTheGioi(this.tileMap, this, master);
//                            break;
                        case 237:
                            ItemLeaveBoss.ItemBossThapVi(tileMap, this);
                            break;
                        default:
                            ItemLeaveBoss.ItemBossThuong(this.tileMap, this, master);
                            break;
                    }
                }
                if (this.tileMap.map.VDMQ()) {
                    for (int i = 0; i < 20; i++) {
                        ItemLeave.leaveYen(this.tileMap, this, master);
                    }
                    if (this.templates.id != 237) {
                        ItemLeaveBoss.ItemBossVDMQ(this.tileMap, this, master);
                    }
                }
                if (this.tileMap.map.LangCo()) {
                    for (int i = 0; i < 20; i++) {
                        ItemLeave.leaveYen(this.tileMap, this, master);
                    }
                    ItemLeaveBoss.ItemBossLangCo(this.tileMap, this, master); // Mob Id : 161 : 162 : 163: 164
                }
                if (this.tileMap.map.cave != null && this.tileMap.map.getXHD() == 9 && ((this.tileMap.map.id == 157 && this.tileMap.map.cave.level == 0) || (this.tileMap.map.id == 158 && this.tileMap.map.cave.level == 1) || (this.tileMap.map.id == 159 && this.tileMap.map.cave.level == 2)) && Util.nextInt(3) < 3) {
