...
public class Char extends Body {
    ...
    public byte taskIndex = 0;
    public int taskCount;
    //
    public int indexTN = -1;
    public Item[] ItemTN = null;
    ...
    public int PointQuyDoi = 0;
    public int naplandau = 0;
    public int nvtruyentin = 0;
    public int top = 0;
    // Vip
    public int vip = -1;
    public int nhanquavip = 0;
    public int diemdanhvip = 0;
    public int danhhieu = 0;
    public int IsLevel6x = 0;
    public int PointMax = 0;
    public int Vnd = 0;
    public int isVIP;
    public int PointBoss = 0;
    public int[] addeff = new int[] { 0, 0, 0, 0 };
    public ArrayList<Short> idSkillBot = new ArrayList<>();
    public ArrayList<Item> ItemGiaHan = new ArrayList<Item>();
    public int TongVnd = 0;
    public int quanaptichluy = 0;
    public int quamocvip = 0;
    public Item[] Fruit = null;
    public int indexGmShop;
    ...
    public synchronized void clearTask() {
        this.taskIndex = -1;
        try {
            for (int i = 0; i < this.p.c.ItemBag.length; ++i) {
                if (this.p.c.ItemBag[i] != null && this.p.c.ItemBag[i].isTypeTask()) {
                    this.p.c.ItemBag[i] = null;
                }
            }
        } catch (Exception ex) {
        }
    ...
    public synchronized void uptaskMaint() {
        final TaskTemplate taskTemplate = Manager.taskTemplates[this.taskId];
        ++this.taskCount;
        if (this.taskCount >= taskTemplate.counts[this.taskIndex]) {
            ++this.taskIndex;
            this.taskCount = 0;
            if (this.taskIndex >= taskTemplate.subNames.length) {
                ++this.taskId;
                this.taskIndex = -1;
    ...
    public Item getIndexFruit(int index) {
        return index < this.Fruit.length && index >= 0 ? this.Fruit[index] : null;
    }
    ...
    public void removeItemBody(byte index) {
        ...
        try {
            ...
            if (index == 26 || index == 29 || index == 31) {
                this.ID_Bien_Hinh = -1;
                this.p.sendInfoMeNewItem();
            }
            if (index == 27) {
                this.ID_MAT_NA = -1;
                this.p.sendInfoMeNewItem();
        ...
        try {
            synchronized (Server.LOCK_MYSQL) {
                ...
                if (red != null && red.first()) {
                    ...
                    nja.FruitUse = red.getByte("FruitUse");
                    ...
                    nja.taskId = (byte) red.getInt("taskId");
                    nja.taskIndex = (byte) red.getInt("taskIndex");
                    nja.taskCount = red.getInt("taskCount");
                    nja.nvtruyentin = red.getInt("nvtruyentin");
                    nja.naplandau = red.getInt("naplandau");
                    nja.vip = red.getInt("vip");
                    nja.isVIP = red.getInt("isVIP");
                    nja.PointMax = red.getInt("PointMax");
                    nja.Vnd = red.getInt("Vnd");
                    nja.PointBoss = red.getInt("PointBoss");
                    nja.IsLevel6x = red.getInt("IsLevel6x");
                    nja.top = red.getInt("top");
                    nja.eventPoint = red.getInt("eventPoint");
                    nja.PointQuyDoi = red.getInt("PointQuyDoi");
                    nja.TongVnd = red.getInt("TongVnd");
                    nja.quanaptichluy = red.getInt("quanaptichluy");
                    nja.quamocvip = red.getInt("quamocvip");
                    ...
                    nja.Fruit = new Item[30];
                    jar = (JSONArray) JSONValue.parse(red.getString("Fruit"));
                    if (jar != null) {
                        for (j = 0; j < jar.size(); ++j) {
                            job2 = (JSONObject) jar.get(j);
                            index = Byte.parseByte(job2.get("index").toString());
                            nja.Fruit[index] = ItemTemplate.parseItem(jar.get(j).toString());
                            job2.clear();
                        }
                    }

                    nja.ItemBody = new Item[32]; // get
                    jar = (JSONArray) JSONValue.parse(red.getString("ItemBody"));
                    if (jar != null) {
                        for (j = 0; j < jar.size(); ++j) {
                            job2 = (JSONObject) jar.get(j);
                            index = Byte.parseByte(job2.get("index").toString());
                            nja.ItemBody[index] = ItemTemplate.parseItem(jar.get(j).toString()); // get
                            job2.clear();
                        }
                    }
                    ...
                    // vĩ thú
                    nja.bijuu = new Item[5];
                    jar = (JSONArray) JSONValue.parse(red.getString("bijuu"));
                    if (jar != null) {
                        for (j = 0; j < jar.size(); ++j) {
                            job2 = (JSONObject) jar.get(j);
                            index = Byte.parseByte(job2.get("index").toString());
                            nja.bijuu[index] = ItemTemplate.parseItem(jar.get(j).toString());
                            job2.clear();
                        }
                    }
                    ...
                    // VIP
                    jar = (JSONArray) JSONValue.parse(red.getString("trangthai"));
                    nja.nhanquavip = Integer.parseInt(jar.get(0).toString());
                    nja.diemdanhvip = Integer.parseInt(jar.get(1).toString());
                    nja.danhhieu = Integer.parseInt(jar.get(2).toString());
                    ...
                    nja.quatanthu = red.getByte("quatanthu");
                ...
                if (red != null && red.first()) {
                    ...
                    if (jar != null) {
                        ...
                        for (index = 0; index < jar.size(); ++index) {
                            ...
                            skill.id = Byte.parseByte(job.get("id").toString());
                            skill.point = Byte.parseByte(job.get("point").toString());
                            nja.skill.add(skill); // get
                            job.clear();
                        }
                    }
                    for (Skill skl : nja.skill) { // get
                    ...
                    nja.ItemBody = new Item[32];
                    jar = (JSONArray) JSONValue.parse(red.getString("ItemBody"));
                    if (jar != null) {
                        for (j = 0; j < jar.size(); ++j) {
                            job2 = (JSONObject) jar.get(j);
                            index = Byte.parseByte(job2.get("index").toString());
                            nja.ItemBody[index] = ItemTemplate.parseItem(jar.get(j).toString());
                            job2.clear();
                        }
                    }

                    nja.ItemMounts = new Item[5]; // get
                    jar = (JSONArray) JSONValue.parse(red.getString("ItemMounts"));
                    if (jar != null) {
                        for (j = 0; j < jar.size(); ++j) {
                            job2 = (JSONObject) jar.get(j);
                            index = Byte.parseByte(job2.get("index").toString());
                            nja.ItemMounts[index] = ItemTemplate.parseItem(jar.get(j).toString()); // get
                            job2.clear();
                        }
                    }

                    // vĩ thú
                    nja.get().bijuu = new Item[5];
                    jar = (JSONArray) JSONValue.parse(red.getString("bijuu"));
                    if (jar != null) {
                        for (j = 0; j < jar.size(); ++j) {
                            job2 = (JSONObject) jar.get(j);
                            index = Byte.parseByte(job2.get("index").toString());
                            nja.get().bijuu[index] = ItemTemplate.parseItem(jar.get(j).toString());
                            job2.clear();
                        }
                    }
    ...
    public void flush() {
        ...
        try {
            synchronized (Server.LOCK_MYSQL) {
                ...
                String sqlSET = "`head`=" + this.head
                        + ",`caiTrang`=" + this.caiTrang
                        + ",`FruitUse`=" + this.FruitUse
                        + ",`rankTDB`=" + this.rankTDB
                        + ",`countWin`=" + this.countWin
                        + ",`countTDB`=" + this.countTDB
                        + ",`isGiftTDB`=" + this.isGiftTDB
                        + ",`taskId`=" + this.taskId
                        + ",`taskIndex`=" + this.taskIndex
                        + ",`taskCount`=" + this.taskCount
                        + ",`class`=" + this.nclass
                        + ",`ppoint`=" + this.ppoint
                        + ",`potential0`=" + this.potential0
                        + ",`potential1`=" + this.potential1
                        + ",`potential2`=" + this.potential2
                        + ",`potential3`=" + this.potential3
                        + ",`spoint`=" + this.spoint
                        + ",`level`=" + this.level
                        + ",`exp`=" + this.exp
                        + ",`expdown`=" + this.expdown
                        + ",`expSkillClone`=" + this.expSkillClone
                        + ",`pk`=" + this.pk
                        + ",`xu`=" + this.xu
                        + ",`yen`=" + this.yen
                        + ",`maxluggage`=" + this.maxluggage
                        + ",`levelBag`=" + this.levelBag
                        + ",`site`='" + jarr.toJSONString()
                        + "', `buyX3` =" + this.countBuyX3
                        + ",`TongVnd`=" + this.TongVnd
                        + ",`quanaptichluy`=" + this.quanaptichluy
                        + ",`quamocvip`=" + this.quamocvip;
                jarr.clear();
                ...
                for (j = 0; j < this.Fruit.length; ++j) {
                    if (this.Fruit[j] != null) {
                        jarr.add(ItemTemplate.ObjectItem(this.Fruit[j], j));
                    }
                }
                sqlSET = sqlSET + ",`Fruit`='" + jarr.toJSONString() + "'";
                jarr.clear();
                ...
                // vĩ thú
                for (j = 0; j < this.bijuu.length; ++j) {
                    if (this.bijuu[j] != null) {
                        jarr.add(ItemTemplate.ObjectItem(this.bijuu[j], j));
                    }
                }

                sqlSET = sqlSET + ",`bijuu`='" + jarr.toJSONString() + "'";
                jarr.clear();
                ...
                jarr.clear();
                jarr.add(this.nhanquavip);
                jarr.add(this.diemdanhvip);
                jarr.add(this.danhhieu);
                sqlSET = sqlSET + ",`trangthai`='" + jarr.toJSONString() + "'";
                jarr.clear();
                ...
                sqlSET = sqlSET
                        + ",`timeRemoveClone` = " + this.timeRemoveClone
                        + ",`isHangDong6x` = " + this.isHangDong6x
                        + ",`ldgtID` = " + this.ldgtID
                        + ",`maxPointCT` = " + this.isTakePoint
                        + ",`clan`='" + jarr.toJSONString()
                        + "',`quatanthu`=" + this.quatanthu
                        + ",`newlogin`='" + Util.toDateString(this.newlogin)
                        + "',`ddClan`=" + this.ddClan
                        + ",`caveID`=" + this.caveID
                        + ",`nCave`=" + this.nCave
                        + ",`pointCave`=" + this.pointCave
                        + ",`useCave`=" + this.useCave
                        + ",`bagCaveMax`=" + this.bagCaveMax
                        + ",`itemIDCaveMax`=" + this.itemIDCaveMax
                        + ",`saveBXH`=" + this.saveBXH
                        + ",`exptype`=" + this.exptype
                        + ",`nvtruyentin`=" + this.nvtruyentin
                        + ",`vip`=" + this.vip
                        + ",`PointMax`=" + this.PointMax
                        + ",`PointBoss`=" + this.PointBoss
                        + ",`IsLevel6x`=" + this.IsLevel6x
                        + ",`eventPoint`=" + this.eventPoint
                        + ",`PointQuyDoi`=" + this.PointQuyDoi
                        + ",`naplandau`=" + this.naplandau
                        + ",`top`=" + this.top + "";
    ...
    public Item getIndexTN(int index) {
        return index < this.ItemTN.length && index >= 0 ? this.ItemTN[index] : null;
    }

    public void GetItemThangNguong() {
        int i = 0;
        this.ItemTN = new Item[18];
        for (Item item : this.get().ItemBody) {
            if (item != null) {
                for (int j = 0; j <= 9; j++) {
                    if (item == this.ItemBody[j] &&
                            item.upgrade >= 14) {
                        this.ItemTN[i] = item;
                        i++;
                    }
    ...
    public int getIndexItemByIdInBag(int id) {
        for (int i = 0; i < this.c.ItemBag.length; i++) {
            Item item = this.c.ItemBag[i];
            if (item != null && item.id == id) {
                return i;
            }
        }
    ...
    public void removeItemBag() throws SQLException {
        byte i;
        for (i = 0; i < this.ItemBag.length; ++i) {
    ...
    // vĩ thú
    public static void doUseBijuu(Player p, Item item, byte index, Message m) {
        p.c.doUseEquipmentBijuu(p, item, index, m);
    }

    public static void doUseEquipmentBijuu(Player p, Item item, byte index, Message m) {
        try {
            byte idM = 4;
            if (ItemTemplate.isTypeEquipmentBijuu(item.id)) {
                idM = (byte) (item.getData().type - 35);
            }
            Item itemM = p.c.get().bijuu[idM];
            if (idM == 4) {
                if (p.c.get().bijuu[0] != null || p.c.get().bijuu[1] != null || p.c.get().bijuu[2] != null
                        || p.c.get().bijuu[3] != null) {
                    p.conn.sendMessageLog("Bạn cần phải tháo trang bị vĩ thú đang sử dụng.");
                    return;
                }
            } else if (p.c.get().bijuu[4] == null) {
                p.conn.sendMessageLog("Bạn cần phải tháo trang bị vĩ thú đang sử dụng.");
                return;
            }
            if (ItemTemplate.isTypeBijuu(item.id) && item.options.size() == 2) {
                RandomOptionsItem.randomOptionvt(item);
            }
            item.isLock = true;
            p.c.ItemBag[index] = itemM;
            p.c.get().bijuu[idM] = item;
            m = new Message(11);
            m.writer().writeByte(index);
            ...
            m.cleanup();
            p.onBijuuInfo(p.c.get().id, p.c.get().bijuu);
            if (!p.c.isTrade) {
    ...
    private void unequipBijuu(int index) {
        byte indexItemBag = p.c.getIndexBagNotItem();
        if (indexItemBag <= 0) {
            this.c.p.conn.sendMessageLog(Language.NOT_ENOUGH_BAG);
        } else if (index >= 0 && index < this.bijuu.length) {
            Item item = this.c.get().bijuu[index];
            if (item != null) {
                if (index == 4 && (this.c.get().bijuu[0] != null || this.c.get().bijuu[1] != null
                        || this.c.get().bijuu[2] != null
                        || this.c.get().bijuu[3] != null)) {
                    p.conn.sendMessageLog("Hãy tháo trang bị của Vĩ thú ra trước!");
                    return;
                }
                for (int i = 0; i < p.c.maxluggage; i++) {
                    if (this.ItemBag[i] == null) {
                        item.isLock = true;
                        this.c.ItemBag[i] = item;
                        this.c.get().bijuu[index] = null;
                        this.p.onBijuuInfo(this.get().id, this.get().bijuu);
                        if (index == 4) {
                            this.p.setBijuu();
                        }
                        Service.CharViewInfo(p, false);
