        ...
        for (i = 0; i < this.map.template.vgo.length; i++) {
            if (p.c.get().x + 100 >= this.map.template.vgo[i].minX) {
                ...
                if (p.c.get().x <= var10001 + 100 && p.c.get().y + 100 >= this.map.template.vgo[i].minY) {
                    ...
                    final Waypoint vg = this.map.template.vgo[i];
                    if (p.c.get().y <= var10001 + 100) {
                        ...
                        //vĩ thú
                        for (n = 0; n < p.c.get().bijuu.length; n++) {
                            if (p.c.get().bijuu[n] != null && p.c.get().bijuu[n].isExpires && p.c.get().bijuu[n].expires < System.currentTimeMillis()) {
                                errornext = 7;
                                break;
                            }
                        }
                        ...
                        if (Task.isLockChangeMap((short) vg.mapid, p.c.taskId)) {
                            errornext = 11;
                        }
                        ...
                        switch (errornext) {
                            ...
                            case 7: {//vĩ thú
                                p.conn.sendMessageLog("Trang bị vĩ thú đã hết hạn. Vui lòng tháo ra để di chuyển");
                                return;
                            }
                            case 11: {
                                GameCanvas.startOKDlg(p.conn, Text.get(0, 84));
                                return;
                            }
                            default: {
                                return;
...
//
                ...
                switch (p.c.tileMap.map.template.id) {
                    case 22:
                       // GameCanvas.SendAutoEffect(p, (byte) 11, 1855, 264, (byte) -1, (short) -1);
                       // GameCanvas.SendAutoEffect(p, (byte) 11, 495, 216, (byte) -1, (short) -1);
                       // GameCanvas.SendAutoEffect(p, (byte) 11, 2319, 192, (byte) -1, (short) -1);
                        break;
                    case 72:
                        GameCanvas.SendAutoEffect(p, (byte) 11, 1427, 552, (byte) -1, (short) -1);
                        GameCanvas.SendAutoEffect(p, (byte) 11, 784, 6648, (byte) -1, (short) -1);
                        GameCanvas.SendAutoEffect(p, (byte) 11, 1039, 672, (byte) -1, (short) -1);
                        break;
                    case 27:
                        GameCanvas.SendAutoEffect(p, (byte) 11, 502, 408, (byte) -1, (short) -1);
                        GameCanvas.SendAutoEffect(p, (byte) 11, 1863, 360, (byte) -1, (short) -1);
                        GameCanvas.SendAutoEffect(p, (byte) 11, 2048, 360, (byte) -1, (short) -1);
                        GameCanvas.SendAutoEffect(p, (byte) 11, 1477, 432, (byte) -1, (short) -1);
                        break;
                    case 1:
                        GameCanvas.SendAutoEffect(p, (byte) 11, 227, 408, (byte) -1, (short) -1);
                        GameCanvas.SendAutoEffect(p, (byte) 11, 940, 408, (byte) -1, (short) -1);
        ...
        try {
            synchronized (this) {
                ...
                if (mob != null) {
                    ...
                    if (mob.templates.id == 237) {
                        mob.hpmax = 100000000 * (map.id - 138);
                        mob.hp = 100000000 * (map.id - 138);
                    }
    ...
    public void sendCharInfo(Player p, Player p2) {
        if (p != null && p.c != null && p2 != null && p2.c != null) {
            ...
            try {
                ...
                //vĩ thú
                if (p.c.get().mobBijuu != null) {
                    m = new Message(-30);
                    m.writer().writeByte(-68);
                    m.writer().writeInt(p.c.get().id);
                    if (p2.conn.version >= 211) {
                        m.writer().writeShort(p.c.get().mobBijuu.templates.id);
                    } else {
                        m.writer().writeByte(p.c.get().mobBijuu.templates.id);
                    }
                    m.writer().writeByte(p.c.get().mobBijuu.isboss ? 1 : 0);
                    m.writer().flush();
                    p2.conn.sendMessage(m);
                    m.cleanup();
                }
        ...
        for (i = 0; i < arNinja.length; ++i) {
            ...
            if (fightChar != null) {
                if (fightChar.get().percentIce() >= Util.nextInt(1, 100)) {
                    ...
                    long damegoc = Util.nextInt((int) p.c.get().dameMin(), (int) p.c.get().dameMax());
                    long dame = Util.nextInt((int) damegoc * 9 / 10, (int) damegoc);
                    ...
                    //vĩ thú
                    int tlxhhtst = fightChar.c.get().getPramItem(162);
                    if (tlxhhtst < Util.nextInt(10000)) {
                        dame -= fightChar.c.get().getPramItem(161);
                    }
                    ...
                    // Né
                    long miss = (long) fightChar.Exactly() * 10000 / fightChar.Miss();
                    ...
                    if (fightChar.c.get().ReactDame() > Util.nextInt(1000)) {
                        p.c.upHP((int) (-dame / 1000));
                        this.attached((int) (odhp - p.c.hp), p.c.id);
                    }
                    fightChar.upHP((int) -dame);
                    this.attached((int) (oldhp - fightChar.hp), fightChar.id);
                    if (fightChar.isDie) {
...
//            }
    ...
    public void handleAfterCloneAttackMob(Mob mob3, CloneCharacter _char) throws IOException {
        if (mob3 != null && _char != null) {
            ...
            if (isfatal) {
                dame *= 2;
                dame = (long) dame * (100 + _char.percentFantalDame()) / 150; // % Sát Thương chí mangj
            ...
            if (mob3.idCharSkill25 == -1 && _char.getEffId(9) != null) {
                this.FireMobSkill25(mob3.id, _char.c.id, (int) (_char.getEffId(9).param * _char.dameSide() / 150));
            }
            if (!mob3.isFire) {
...
// set dame quái
    ...
    public void updatePlayer() throws IOException {
        ...
        for (i = this.players.size() - 1; i >= 0; i--) {
            ...
            if (p.c.isVIP == 3 && p.c.ID_NAME != 3003){
                p.c.ID_NAME = 3003;
                Service.CharViewInfo(p, true);  
            } else if (p.c.isVIP == 2 && p.c.ID_NAME != 3002){
                p.c.ID_NAME = 3002;
                Service.CharViewInfo(p, true);  
            }else if (p.c.isVIP == 1 && p.c.ID_NAME != 3001){
                p.c.ID_NAME = 3002;
                Service.CharViewInfo(p, true);  
            }else if (p.c.isVIP == 0 && p.c.ID_NAME > 3000){
                p.c.ID_NAME = -1;
                Service.CharViewInfo(p, true);  
            }
            ...
            if (p.c.getPramItem(152) >= Util.nextInt(1, 2)) {
                List<Mob> mobs = getMonsterByDistance(200, p);
                for (Mob mob : mobs) {
                    int dame = ( mob.hp) / 30;
                    if(!mob.isboss && !p.c.isDie){
                        for (int f = 0; f < mobs.size(); f++) {
                            GameCanvas.addEffect(p.conn, (byte) 1, mobs.get(f).id, (byte) 101, 500, 1, true);
                            this.attachedMob(dame, mobs.get(f).id, false);
                ...
                for (i = 0; i < this.itemMap.size(); i++) {
                    ...
                    if (itemmap != null) {
                        if (this.itemMap.get(i).itemMapId == itemmapid) {
                            ...
                            if (itemmap.checkMob == 4 && p.c.level < 40) {
                                p.sendAddchatYellow("Yêu cầu level 40 mới nhặt đc item boss.");
                                return;
                            }
                            if (itemmap.checkMob != 4) {
    ...
    public void AddEffectItem(Player p) {
        ...
        if (p.c.get().ItemBody[10] != null && p.c.get().ItemBody[10].id == 1144) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 238, 1000, 1000, false);
            }
        }
        ...
        if (p.c.get().ItemBody[21] != null && p.c.get().ItemBody[21].id == 710) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 184, 1000, 500, false);
            }
        }
        ...
        if (p.c.id == -1 || p.c.id == 1) {
            for (k = 0; k < this.players.size(); k++) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 197, 1000, 0, false);
            }
        }
        if (p.c.id == -1 || p.c.id == -1) {
            for (k = 0; k < this.players.size(); k++) {
        ...
        if (p.c.get().ItemBody[14] != null && p.c.get().ItemBody[14].id == 906) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 106, 1000, 10, false);
            }
        }
        if (p.c.get().ItemBody[14] != null && p.c.get().ItemBody[14].id == 907) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 105, 1000, 10, false);
            }
        }
        if (p.c.get().ItemBody[14] != null && p.c.get().ItemBody[14].id == 908) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 99, 1000, 10, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.DE_NHAT) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 203, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.DE_NHI) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 204, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.DE_TAM) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 205, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.DE_TU) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 206, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.CHI_TON) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 210, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.SADBOY) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 208, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_DAI_GIA_BLUE) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 211, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_BA_CHU_LOI_DAI) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 212, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_DAI_GIA_TIM) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 213, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_VUA_LOAN_DAU) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 214, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_VUONG_GIA1) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 216, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_GT_HUYEN_THOAI) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 217, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_TRUM_VQMM) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 218, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_DUY_NGA_DOC_TON) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 219, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_TOP_BOSS) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 220, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_TY_PHU) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 221, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_GT_MINH_MAN) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 222, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_GT_SIENG_NANG) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 223, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_KNIGHTMASTER) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 224, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_KING_VIP) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 225, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[31] != null && p.c.get().ItemBody[31].id == ItemName.BINH_THU_KING_SAVEN) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 226, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[26] != null && p.c.get().ItemBody[26].id == 945) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 97, 1000, 1000, false);
            }
        }
        if (p.c.get().ItemBody[15] != null && p.c.get().ItemBody[15].id == 935) {
            for (k = this.players.size() - 1; k >= 0; k--) {
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 5, 1000, 1000, false);
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 8, 1000, 1000, false);
                GameCanvas.addEffect((this.players.get(k)).conn, (byte) 0, p.c.get().id, (short) 11, 1000, 1000, false);
            }
        }
        if (p.c.getPramItem(152) >= Util.nextInt(1, 2)) {
            List<Mob> mobs = getMonsterByDistance(200, p);
