                ...
                if (red != null && red.first()) {
                    ...
                    if (jar != null) {
                        for(j = 0; j < jar.size(); ++j) {
                            job2 = (JSONObject)jar.get(j);
                            index = Byte.parseByte(job2.get("index").toString());
                            cl.bijuu[index] = ItemTemplate.parseItem(jar.get(j).toString());
                            job2.clear();
                        }
                    }
                    ...
                    red.close();
                    //vĩ thú
                    SQLManager.stat.executeUpdate("INSERT INTO clone_ninja(`id`,`name`,`ItemBody`,`ItemMounts`,`bijuu`) VALUES (" + (-10000000 - n.id) + ",'" + n.name + "','[]','[]','[]');");
                    cl = new CloneCharacter(n);
                    cl.id = -10000000 - n.id;
                    cl.speed = 8;
