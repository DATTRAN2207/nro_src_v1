...
public class UseItem {
    ...
    public static short[] HanSuDung1Thg = new short[]{30};
    ...
    public static void uesItem(Player p, Item item, byte index) {
        ...
        try {
            ...
            if (ItemTemplate.isTypeBijuu(item.id)) {
                p.c.doUseBijuu(p, item, index, m);
            } else if (ItemTemplate.isTypeEquipmentBijuu(item.id)) {
                ...
                return;
            } else if (ItemTemplate.isTypeBody(item.id)) {
                ...
                m.cleanup();
                if ((item.id >= 795 && item.id <= 805) || (item.id >= 813 && item.id <= 817) || (item.id >= 825 && item.id <= 826) || (item.id >= 829 && item.id <= 830 || item.id == 839 || item.id >= 843 && item.id <= 846 || (item.id >= 858 && item.id <= 866) || item.id == 892 || (item.id >= 881 && item.id <= 883) || item.id == 894 || item.id == 710 || item.id == 914 || item.id == 945)) {
...
//                    }
                    ...
                    case ItemName.DIEU_GIAY:
                        ItemHandleEvent.DieuGiay(p, item, index);
                        break;
                    case ItemName.DIEU_VAI:
                        ItemHandleEvent.DieuVai(p, item, index);
                        break;
                    ...
                    case ItemName.HOP_MA_QUY:
                        ...
                        break;
                    case ItemName.GIAY_THONG_HANH:
                        ItemHandleEvent.GiayThongHanh(p, item, index);
                        break;
                    case ItemName.DA_MA_THUAT:
                        ItemHandleEvent.DaMaThuat(p, item, index);
                        break;
                    case ItemName.RUONG_NGOC:
                        ItemHandle.RuongNgoc(p, item, index);
                        break;
                        case ItemName.BANH_KHUC_CAY_DAU_TAY:
                        ItemHandleEvent.BanhKhucDauTay(p, item, index);
                        break;
                    case ItemName.BANH_KHUC_CAY_CHOCOLATE:
                        ItemHandleEvent.BanhKhucChocolate(p, item, index);
                        break;
                    case ItemName.BANH_CHUNG:
                        ItemHandleEvent.BanhChung(p, item, index);
                        break;
                    case ItemName.BANH_TET:
                        ItemHandleEvent.BanhTet(p, item, index);
                        break;
                    case 548:
                    ...
                    case 891:
                        ...
                        break;
                    case 944:
                        ItemHandle.Ruong12x(p, item, index);
                        break;
                    case 279:
                    ...
                    case ItemName.BOMB:
                    case ItemName.CONTROL:
                    case ItemName.DARK:
                    case ItemName.DRAGON:
                    case ItemName.FALCON:
                    case ItemName.GRAVITY:
                    case ItemName.ICE:
                    case ItemName.LEOPARD:
                    case ItemName.MAMMOTH:
                    case ItemName.QUAKE:
                    case ItemName.RUMBLE:
                    case ItemName.SPIRIT:
                    case ItemName.BLIZZARD:
                    case ItemName.DIAMOND:
                    case ItemName.DOUGHIN:
                    case ItemName.FLAME:
                    case ItemName.KITSUNE:
                    case ItemName.LIGHT:
                    case ItemName.LOVE:
                    case ItemName.MAGMA:
                    case ItemName.PORTAL:
                    case ItemName.REVIVE:
                    case ItemName.ROCKET:
                    case ItemName.RUBBER:
                    case ItemName.SAND:
                    case ItemName.SHADOW:
                    case ItemName.SOUND:
                    case ItemName.SPIDER:
                    case ItemName.T_REX:
                    case ItemName.VENOM:{
                        DevilFruit.allFruitUse(p, item, index);
                        break;
                    }
                    case 733:
                    case 734:
                    ...
                    case 905: {
                        Service.sendInputDialog(p, (short) -100, "nhập tên muổn đổi");
                        break;
                    }
                    case 760:
                    case 761:
                    ...
                    // vĩ thú
                    case ItemName.TRUNG_VI_THU: {
                        if (p.c.getBagNull() == 0) {
                            p.sendAddchatYellow("Hành trang không đủ chỗ trống");
                            return;
                        }
                        if (p.c.yen < 1000000) {
                            p.sendAddchatYellow("Bạn không đủ 1 triệu yên");
                            return;
                        }
                        if (Util.nextInt(100) > 5) {
                            Item itm = ItemTemplate.itemDefault(ItemName.TAMAMIZU);
                            if (Util.nextInt(100) < 1) {
                                Manager.chatKTG(p.c.name + " đã mở " + data.name + " đã nhận được " + itm.getData().name + " vĩnh viến");
                            } else {
                            ...
                            p.sendAddchatYellow("Trứng đã bị thối rồi thật đáng tiếc");
                        }
                        p.c.removeItemBag(index, 1);
                        p.c.upyenMessage(-1000000);
                        break;
                    }
                    case ItemName.TAMAMIZU: {
                        if (p.c.getBagNull() == 0) {
                            p.sendAddchatYellow("Hành trang không đủ chỗ trống");
                            return;
                        }
                        Item itm = ItemTemplate.itemDefault(Util.nextInt(1070, 1087));
                        if (item.isExpires == true) {
                            itm.setUpgrade(100);
                            itm.sys = 4;
                            int HSD = HanSuDungNew[Util.nextInt(HanSuDungNew.length)];
                            itm.isExpires = true;
                    ...
                    case ItemName.TAMAKIRO: {
                        if (p.c.getBagNull() == 0) {
                            p.sendAddchatYellow("Hành trang không đủ chỗ trống");
                            return;
                        }
                        p.c.removeItemBag(index, 1);
                        Item itm = ItemTemplate.itemDefault(Util.nextInt(1088, 1105));
                        if (item.isExpires == true) {
                            itm.setUpgrade(100);
                            itm.sys = 4;
                            int HSD = HanSuDungNew[Util.nextInt(HanSuDungNew.length)];
                    ...
                    case ItemName.TAMAMURA: {
                        if (p.c.getBagNull() == 0) {
                            p.sendAddchatYellow("Hành trang không đủ chỗ trống");
                            return;
                        }
                        p.c.removeItemBag(index, 1);
                        Item itm = ItemTemplate.itemDefault(Util.nextInt(1106, 1123));
                        if (item.isExpires == true) {
                            itm.setUpgrade(100);
                            itm.sys = 4;
                            int HSD = HanSuDungNew[Util.nextInt(HanSuDungNew.length)];
                    ...
                    case ItemName.TAMADAI: {
                        if (p.c.getBagNull() == 0) {
                            p.sendAddchatYellow("Hành trang không đủ chỗ trống");
                            return;
                        }
                        p.c.removeItemBag(index, 1);
                        Item itm = ItemTemplate.itemDefault(Util.nextInt(1124, 1141));
                        if (item.isExpires == true) {
                            itm.setUpgrade(100);
                            itm.sys = 4;
                            int HSD = HanSuDungNew[Util.nextInt(HanSuDungNew.length)];
                    ...
                    case ItemName.RUONG_PHU_KIEN_VI_THU: {
                        if (p.c.getBagNull() == 0) {
                            p.sendAddchatYellow("Hành trang không đủ chỗ trống");
                            return;
                        }
                        p.c.removeItemBag(index, 1);
                        Item itm = ItemTemplate.itemDefault(Util.nextInt(1023, 1058));
                        itm.setLock(true);
                        p.c.addItemBag(false, itm);
                        break;
                    }
                    case ItemName.QUA_CHAKA_XANH: {
                        if (p.addExpForVT(10)) {
                            p.c.removeItemBag(index, 1);
                        }
                        break;
                    }
                    case ItemName.QUA_CHAKA_VANG: {
                        if (p.addExpForVT(200)) {
                            p.c.removeItemBag(index, 1);
                        }
                        break;
                    }
                    case ItemName.QUA_CHAKA_TIM: {
                        if (p.addExpForVT(300)) {
                            p.c.removeItemBag(index, 1);
                        }
                        break;
                    }
                    case ItemName.NGU_HANH_QUA: {
                        if (p.upgradeVT()) {
                            p.c.removeItemBag(index, 1);
                        }
                        break;
                    }
                    case ItemName.NGU_HANH_THACH: {
                        if (p.evolveBijuu()) {
                            p.c.removeItemBag(index, 1);
                        }
                        break;
