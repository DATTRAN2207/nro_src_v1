...
//                        }
            ...
            case 1: {
                ...
                switch (b3) {
                    ...
                    case 4: {
                        ...
                        Service.startYesNoDlg(p, (byte) 99,
    ...
    public static void npcDevilFruit(Player p, byte npcid, byte menuId, byte b3) {
        try {
            if (p.c.isNhanban) {
                Service.chatNPC(p, (short) npcid, Language.NOT_FOR_PHAN_THAN);
                return;
            ...
            switch (menuId) {
                case 0: {
                    ...
                    if (p.luong < 1000) {
                        Service.chatNPC(p, (short) npcid, "Ngươi không đủ 1000 lượng!");
                        return;
                    }
                    if (p.c.getBagNull() == 0) {
                    ...
                    DevilFruit.RandomFruit(p);
                    p.upluongMessage(-1000);
                    break;
                }
                case 1: {
                    DevilFruit.openMenuFruit(p);
                    break;
                }
                case 2: {
                    if (p.c.FruitUse != -1) {
                        if (p.luong < 10000) {
                            Service.chatNPC(p, (short) npcid, "Ngươi không đủ 10000 lượng!");
                            return;
                        }
                        p.c.FruitUse = -1;
                        ...
                        p.endLoad(true);
                        p.conn.sendMessageLog("Xóa trái ác quỷ thành công");
                        p.upluongMessage(-10000);
                    } else {
                        p.conn.sendMessageLog("Bạn đang không sử dùng trái ác quỷ nào");
                    }
                    break;
                }
                case 3: {
                    if (p.c.FruitUse != -1) {
                        ...
                        p.conn.sendMessageLog("Bạn đang sử dùng trái ác quỷ " + item.name);
                    } else {
                        p.conn.sendMessageLog("Bạn đang không sử dùng trái ác quỷ nào");
                    }
                    break;
                ...
                case 4: {
                    if (p.c.FruitUse != -1) {
                        ...
                        p.conn.sendMessageLog("sử dùng trái ác quỷ để thức tỉnh");
                    }
                    break;
                }
                case 5: {
                    Server.manager.sendTB(p, "Fruits Tier List",
                            "Để thức tỉnh trái ác quỷ thì bạn phải sử dụng trái ác quỷ đó và thu thập đủ 10 trải cùng loại.\n\n"
                            + "Cấp Mythical gồm: Kitsune, Leopard, Dragon, Shadow, Spirit, Dark, Blizzard, T-Rex, Mammoth.\n\n"
                            + "Cấp Legendary gồm: Rumble, Control, Quake, Ice, Revive, Venom, Spider, Dough, Magma.\n\n"
                            + "Cấp Epic gồm: Portal, Sand, Gravity, Light, Flame.\n\n"
                            + "Cấp Rare gồm: Love, Rubber, Sound, Diamond.\n\n"
                            + "Cấp Uncommon gồm: Falcon, Bomb, Rocket.\n\n"
                            + "Tỷ lệ random ra cấp Uncommon là 45%.\n"
                            + "Tỷ lệ random ra cấp Rare là 30%.\n"
                            + "Tỷ lệ random ra cấp Epic là 20%.\n"
                            + "Tỷ lệ random ra cấp Legendary là 4%.\n"
                            + "Tỷ lệ random ra cấp Mythical là 1%.\n"
                            + "Random tiêu tốn 1.000 lượng.\n"
                            + "Xóa trái ác quỷ đang dùng tiêu tốn 10.000 lượng.\n"
                        );
                    break;
                }
                default: {
...
//            }
            switch (menuId) {
                ...
                case 4: {
                    p.c.ItemGiaHan.clear();
                    for (int i = 0; i < p.c.ItemBag.length; i++) {
                        Item imb = p.c.ItemBag[i];
                        if (imb != null && imb.isExpires == true && imb.expires > 0) {
                            p.c.ItemGiaHan.add(imb);
                        }
                    }
    ...
    public static void npcTajima(Player p, byte npcid, byte menuId, byte b3) {
        switch (menuId) {
            case 0:
                    Server.manager.sendTB(p, "BXH Đại Gia", Rank.getStringBXH(7));
                break;
            case 1:
                if (p.c.taskIndex == -1) {
                ...
                switch (b3) {
                    ...
                    case 3: {
                        Server.manager.sendTB(p, "Hướng dẫn", "Chiến trường được mở 2 lần mỗi ngày.\n"
                                + "- Chiến trường lv30 - 50: giành cho nhân vật level từ 30 đến 50, điểm danh vào lúc 12h và bắt đầu từ 12h10' đến 13h00'.\n"
                                + "- Chiến trường trên lv100: giành cho nhân vật level từ 100 trở lên, điểm danh vào lúc 21h và bắt đầu từ 21h10' đến 21h40'.\n\n");
                        break;
                    }
                }
                break;
    ...
    public static void npcGoosho(Player p, byte npcid, byte menuId, byte b3) {
        switch (menuId) {
            ...
            case 4:
                switch (b3) {
                    case 0: {
                        GmShop.openMenuGmShop(p);
                        break;
                    }
                    case 1: {
                        p.conn.sendMessageLog("Tên nhân vật " + p.c.name + "\ncoin hiện tại " + p.coin);
                        break;
                    }
                    default: {
                        break;
    ...
    public static void npcShinwa(Player p, byte npcid, byte menuId, byte b3) throws IOException {
        if (p.c.xu < 10000000) {
            p.conn.sendMessageLog("Yêu Cầu Trên 10 Triệu Xu Mới Có Thể Dùng Chức Năng Này");
            return;
        }
        switch (menuId) {
            case 0: {
            ...
            case 4: {
                switch (b3) {
                    case 0:
                        ...
                        break;
                    case 1:
                        Service.sendInputDialog(p, (short) 555, "Kiếm");
                        break;
                    case 2:
                        Service.sendInputDialog(p, (short) 556, "Tiêu");
                        break;
                    case 3:
                        try {
                            String a = "";
    ...
    public static void npcKanata_LoiDai(Player p, byte npcid, byte menuId, byte b3) {
        switch (menuId) {
            ...
            case 1:
                ...
                Service.sendInputDialog(p, (short) 3, "Đặt tiền cược (lớn hơn 10000 xu và chia hết cho 50)");
                break;
            case 2:
            ...
            case 1: {
                switch (b3) {
                    case 0: {
                        ...
                        if(p.c.TongVnd >= 500000 && p.c.quanaptichluy == 0){
                            //p.upluongMessage(300000);
                          //  p.c.upxuMessage(1000000000);
                          //  p.conn.sendMessageLog("Bạn đã nhận quà mốc 500.000 thành công");
                         //   p.c.quanaptichluy = 1;
                        } else {
                          //  p.conn.sendMessageLog("Bạn Đã Nhận Hoặc Chưa Nạp Đủ 500.000 Để Nhận Thưởng \n Số Tiền Tích Luỹ Của Bạn : " + Util.getFormatNumber(p.c.TongVnd));
                        }
                        break;
                    }
                    case 1: {
                        ...
                        if(p.c.TongVnd >= 1000000 && p.c.quanaptichluy == 1){
                          //  p.upluongMessage(700000);
                        //    p.c.upxuMessage(2000000000);
                         //   p.conn.sendMessageLog("Bạn đã nhận quà mốc 1.000.000 thành công");
                         //   p.c.quanaptichluy = 2;
                        } else {
                            //  p.conn.sendMessageLog("Bạn Đã Nhận Hoặc Chưa Nạp Tích Luỹ Đủ 1.000.000 Để Nhận Thưởng \n Hoặc Bạn Chưa Nhận Mốc 500.000 \n Số Tiền Tích Luỹ Của Bạn : " + Util.getFormatNumber(p.c.TongVnd));
                        }
                        break;
                    }
                    case 2: {
                        ...
                        if(p.c.TongVnd >= 2000000 && p.c.quanaptichluy == 2){
                           // Item mn = ItemTemplate.itemDefault(p.c.gender == 1 ? 306 : 307);
                           // mn.isExpires = false;
                           // mn.expires = -1L;
                           // mn.options.clear();
                           // mn.options.add(new Option(0, 1000));
                           // mn.options.add(new Option(1, 1000));
                           // mn.options.add(new Option(8, 120));
                           // mn.options.add(new Option(9, 120));
                          //  mn.options.add(new Option(10, 500));
                          //  mn.options.add(new Option(87, 10000));
                           // mn.options.add(new Option(57, 100));
                          //  mn.options.add(new Option(58, 20));
                          //  mn.options.add(new Option(88, 2500));
                           // mn.options.add(new Option(89, 2500));
                          //  mn.options.add(new Option(90, 2500));
                           // mn.options.add(new Option(85, 9));
                           // mn.options.add(new Option(78, 300));
                          //  mn.options.add(new Option(100, 50));
                           // mn.options.add(new Option(94, 5));
                           // mn.options.add(new Option(69, 300));
                          //  mn.options.add(new Option(101, 5));
                           // mn.options.add(new Option(102, 5000));
                          //  mn.options.add(new Option(103, 2000));
                          //  mn.options.add(new Option(67, 95));
                           // mn.upgrade = 16;
                           // p.c.addItemBag(false, mn);
                           // p.upluongMessage(1000000);
                          //  p.c.quanaptichluy = 3;
                        } else {
                            //  p.conn.sendMessageLog("Bạn Đã Nhận Hoặc Chưa Nạp Tích Luỹ Đủ 2.000.000 Để Nhận Thưởng \n Hoặc Bạn Chưa Nhận Mốc 1.000.000 \n Số Tiền Tích Luỹ Của Bạn : " + Util.getFormatNumber(p.c.TongVnd));
                        }
                        break;
                    }
                    case 3: {
                        ...
                        if(p.c.TongVnd >= 5000000 && p.c.quanaptichluy == 3){
                           // Item quanphep = ItemTemplate.itemDefault(935);
                         ...
                         //   p.upluongMessage(3000000);
                         //   p.conn.sendMessageLog("Bạn đã nhận quà mốc 5.000.000 thành công");
                          //  p.c.quanaptichluy = 4;
                        } else {
                             // p.conn.sendMessageLog("Bạn Đã Nhận Hoặc Chưa Nạp Tích Luỹ Đủ 5.000.000 Để Nhận Thưởng \n Hoặc Bạn Chưa Nhận Mốc 2.000.000 \n Số Tiền Tích Luỹ Của Bạn : " + Util.getFormatNumber(p.c.TongVnd));
                        }
                        break;
                    }
                    case 4: {
                        Server.manager.sendTB(p, "Hướng Dẫn",
                            "Mốc 500k: nhận được 300.000 lượng \n"
                            + "cộng thêm 1 tỷ xu\n\n"
                            + "Mốc 1 triệu: nhận được 700.000 lượng \n"
                            + "cộng thêm 2 tỷ xu\n\n"
                            + "Mốc 2 triệu: nhận được 1.000.000 lượng \n"
                            + "cộng thêm  1 mặt nạ hannya + 16 vĩnh viễn\n\n"
                            + "Mốc 5 triệu: nhận được 3.000.000 lượng \n"
                            + "cộng thêm  1 Quân Pháp + 16 vĩnh viễn\n\n"
                        );
                    }
                }
                break;
...
//                        "- MUA TRANG BỊ , VIP , XEM CHỈ SỐ TRANG BỊ , XU LƯỢNG INBOX ZALO ADMIN : 0944041678 .\n"
//                );
//                break;
//            case 4: {
//                Item Item = p.c.ItemBody[14];
...
//            }
    ...
    public static void KhaiHoa(Player p, byte npcid, byte menuId, byte b3) {
        if (p.c.isNhanban) {
            Service.chatNPC(p, (short) npcid, Language.NOT_FOR_PHAN_THAN);
            return;
        }
        switch (menuId) {
            case 0: {
                Service.openMenuThangNguong(p);
                break;
            }
            case 1: {
                Service.openMenuThangCap(p);
                break;
            }
        }
    }
    public static void npcKizzz(Player p, byte npcid, byte menuId, byte b3) {
        if (p.c.isNhanban) {
            Service.chatNPC(p, (short) npcid, Language.NOT_FOR_PHAN_THAN);
            return;
        }
...
//                if (p.c.isVIP == 1 && p.c.quamocvip == 0) {
                   ...
                   // antoc.options.add(new Option(87, 10000));
                  ...
                  //  thuhon.options.add(new Option(82, 5000));
                   // thuhon.options.add(new Option(87, 10000));
                ...
                //    thuhon.options.add(new Option(74, 300));
                  ...
                  //  thuhon.options.add(new Option(81, 200));
                   // thuhon.options.add(new Option(76, 5000));
                   // thuhon.upgrade = 16;
                  //  Item aodai = ItemTemplate.itemDefault(p.c.gender == 1 ? 843 : 844);
                   // aodai.options.add(new Option(85, 9));
                   // aodai.options.add(new Option(87, 10000));
                   // aodai.options.add(new Option(69, 100));
                  //  aodai.options.add(new Option(57, 100));
                    //aodai.options.add(new Option(58, 10));
                   // aodai.options.add(new Option(30, 1000));
                  //  aodai.options.add(new Option(31, 10));
                   // aodai.options.add(new Option(32, 2000));
                   // aodai.options.add(new Option(36, 500));
                 //   aodai.options.add(new Option(53, 1000));
                   // aodai.options.add(new Option(51, 1000));
                   // aodai.options.add(new Option(52, 1000));
                   // aodai.options.add(new Option(100, 30));
                   // aodai.upgrade = 16;
                  //  p.c.addItemBag(false, antoc);
                   // p.c.addItemBag(false, thuhon);
                   // p.c.addItemBag(false, aodai);
                  //  p.conn.sendMessageLog("Bạn đã nhận quà mốc [VIP] thành công");
                   // p.c.quamocvip = 1;
//                } else {
...
//                        if (p.c.isVIP == 2 && p.c.quamocvip == 1) {
                          //  Item tbdq = ItemTemplate.itemDefault(894);
                          ...
                          //  tbdq.options.add(new Option(76, 10000));
                           ...
                           // tbdq.upgrade = 16;
                            //Item ktv = ItemTemplate.itemDefault(945);
                          ...
                          //  ktv.upgrade = 16;
                          // Item gsmr = ItemTemplate.itemDefault(p.c.gender == 1 ? 845 : 846);
                           // gsmr.options.add(new Option(73, 15));
                           // gsmr.options.add(new Option(86, 500));
                           // gsmr.options.add(new Option(84, 500));
                          //  gsmr.options.add(new Option(69, 500));
                          //  gsmr.options.add(new Option(8, 100));
                           // gsmr.options.add(new Option(9, 100));
                          //  gsmr.options.add(new Option(57, 200));
                           // gsmr.options.add(new Option(58, 20));
                           // gsmr.options.add(new Option(85, 9));
                          //  gsmr.options.add(new Option(6, 10000));
                          //  gsmr.options.add(new Option(7, 1000));
                          //  gsmr.options.add(new Option(88, 5000));
                          //  gsmr.options.add(new Option(89, 5000));
                           // gsmr.options.add(new Option(90, 5000));
                          //  gsmr.upgrade = 16;
                          //  p.c.addItemBag(false, tbdq);
                          //  p.c.addItemBag(false, ktv);
                          //  p.c.addItemBag(false, gsmr);
...
//                        if (p.c.isVIP == 3 && p.c.quamocvip == 2) {
                           // Item mnst = ItemTemplate.itemDefault(p.c.gender == 1 ? 273 : 274);
...
//                        }
    ...
    public static void npcChanle(Player p, byte npcid, byte menuId, byte b3) {
        if (p.c.isNhanban) {
            Service.chatNPC(p, (short) npcid, Language.NOT_FOR_PHAN_THAN);
            return;
        }
        if (Server.manager.baotri >= 1) {
            p.conn.sendMessageLog ("Máy chủ chuẩn bị bảo trì vui lòng thử lại sau");
            return;
        }
        switch (menuId) {
            case 0: {
                switch (b3) {
                    case 0:
                        Service.sendInputDialog(p, (short) -70, "Nhập số xu");
                        break;
                    case 1:
                        Service.sendInputDialog(p, (short) -69, "Nhập số xu");
                        break;
                    case 2:
                        Service.sendInputDialog(p, (short) -71, "Nhập số lượng");
                        break;
                    case 3:
                        Service.sendInputDialog(p, (short) -72, "Nhập số lượng");
                        break;
                    case 4:
                        Server.manager.sendTB(p,"Hướng dẫn",
                        "Mỗi lần cược bạn sẽ nhận một số ngẫu nhiên từ 0 đến 9\n"
                        + "Nếu thắng sẽ nhận 100% số tiền đã cược\n"
                        + "Nếu thua sẽ mất đi số tiền đã cược");
                        break;
                }
                break;
            }
