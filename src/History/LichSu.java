...
public class LichSu {
    ...
    public static void Lichsudoiten (String name1, Player p){
        try {
            Calendar calender = Calendar.getInstance();
            Date date = calender.getTime();
            LichSu.dateFormatFile = new SimpleDateFormat(DATE_FORMAT_FILE);
            String str = Util.toDateString(Date.from(Instant.now()));// code by thành nguyễn
            String filename = "LichSu/lichSuDoiTen/LichSuDoiTen_" + dateFormatFile.format(date) + ".txt";
            FileWriter fw = new FileWriter(filename,true);
            BufferedWriter bw = new BufferedWriter(fw);
            bw.write("Name : " + p.c.name +" đã đổi tên thành "+ name1 +"\n");
            bw.close();
            fw.close();
        } catch (Exception e) {
            System.out.println(e);
    ...
    public static void LichSuChanLeLuong(String name, String noidung, int luongcu, int luongmoi, int luongthaydoi) {
        try {
            Calendar calender = Calendar.getInstance();
            Date date = calender.getTime();
            LichSu.dateFormatFile = new SimpleDateFormat(DATE_FORMAT_FILE);
            String str = Util.toDateString(Date.from(Instant.now()));
            String filename = "LichSu/ChanLe/LichSuChanLeLuong/LichSuChanLeLuong_" + dateFormatFile.format(date) + ".txt";
            FileWriter fw = new FileWriter(filename, true);
            BufferedWriter bw = new BufferedWriter(fw);
            bw.write("----------Thời Gian : " + str + "--------\n"
                    + "- Name : " + name + "\n"
    ...
    public static void LichSuChanLeXu(String name, String noidung, int xucu, int xumoi, int xuthaydoi) {
        try {
            Calendar calender = Calendar.getInstance();
            Date date = calender.getTime();
            LichSu.dateFormatFile = new SimpleDateFormat(DATE_FORMAT_FILE);
            String str = Util.toDateString(Date.from(Instant.now()));
            String filename = "LichSu/ChanLe/LichSuChanLeXu/LichSuChanLeXu_" + dateFormatFile.format(date) + ".txt";
            FileWriter fw = new FileWriter(filename, true);
            BufferedWriter bw = new BufferedWriter(fw);
            bw.write("----------Thời Gian : " + str + "--------\n"
                    + "- Name : " + name + "\n"
