...
public class ItemHandle {
    ...
    public static short[] idItemRuong12x = new short[]{915, 916, 917, 918, 919, 920, 921, 922, 923, 924, 925, 926, 927, 928, 929, 930, 931, 932, 933, 934};
    ...
    public static void Ruong12x(Player p, Item item, byte index) {
        if (p.c.getBagNull() == 0) {
            p.conn.sendMessageLog("Hành trang không đủ chỗ trống.");
            return;
        }
        ...
        if (Util.nextInt(10) < 9) {
           // p.c.upyenMessage(Util.nextInt(10000, 100000));
            ...
            short idI = idItemRuong12x[Util.nextInt(idItemRuong12x.length)];
            ItemTemplate data2 = ItemTemplate.ItemTemplateId(idI);
            Item itemup;
            if (data2.type < 10) {
                if (data2.type == 1) {
